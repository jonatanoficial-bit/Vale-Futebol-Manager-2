export function validateBetaProfessionalSystem(snapshot={}){
  const errors = [];
  const warnings = [];
  if(snapshot.version !== 'v8.0.0') errors.push('Versão Beta Profissional divergente de v8.0.0.');
  if(Number(snapshot.schema || 0) !== 800) errors.push('Schema 800 ausente no Beta Profissional.');
  ...
  return { ok:errors.length===0, status:errors.length?'error':warnings.length?'warning':'ok', errors, warnings, phase:'v8.0.0-beta-professional-consolidation', checkedAt:'runtime' };
}
