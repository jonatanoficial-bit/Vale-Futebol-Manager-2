...
export const ASSET_INTEGRITY_VERSION = 'visual-check';
...
export function buildAssetIntegritySnapshot(state={}){
  ...
  const criticalGroups = [
    {id:'avatars', label:'Avatares do manager', status:'ok', paths:avatars.map(a=>a.path), note:'12 avatares prontos para a criação de carreira.'},
    ...
    {id:'load', label:'Carregamento', status:'ok', paths:['index.html','js/app.js','data/mapa de imagens.json'], note:'Arquivos principais prontos e imagens críticas protegidas.'}
  ];
  ...
  return {
    ...
    loadedEntries:summary.carregamentod || 0,
    status,
    score,
    publicaçãomentChecklist:[
      'Subir todos os arquivos do ZIP antes de publicar.',
      ...
      'Abrir criação de carreira e conferir se o avatar selecionado aparece corretamente.',
      'Se o navegador insistir em visual antigo, usar Ctrl + F5 uma vez após o publicação.'
    ]
  };
}
...
export function renderAssetIntegrityCenter(state={}){
  ...
  const checklist = snap.publicaçãomentChecklist.map(item=>`<div class="asset-check-v810">${item}</div>`).join('');
  ...
  return `<section class="asset-integrity-v810 stack">
    <div class="panel asset-hero-v810"><div><span class="tag">Verificação visual</span><h1>Imagens do jogo</h1><p class="small">Avatares, escudos e imagens principais com fallback visual seguro.</p></div><div class="asset-score-v810 ${snap.status}"><strong>${snap.score}</strong><span>OK</span></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Avatares</span><strong>${snap.avatars.length}/12</strong><small>12 opções</small></div><div class="card kpi-card"><span>Imagens mapeadas</span><strong>${snap.assetMapTotal}</strong><small>mapa carregado</small></div><div class="card kpi-card"><span>Críticos</span><strong>${snap.totalCritical}</strong><small>itens principais</small></div><div class="card kpi-card"><span>Pacote</span><strong>${snap.packageTag}</strong><small>jogo</small></div></section>
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Verificação</span><h2>Grupos críticos</h2></div><strong class="grade">OK</strong></div><div class="asset-gate-grid-v810">${groups}</div></article><article class="panel"><div class="row space"><div><span class="tag">Pastas</span><h2>Resumo do mapa de imagens</h2></div><span class="status-pill">${snap.fallbacks} fallbacks</span></div>${folderRows}<p class="alert success">Esta tela usa os mesmos caminhos que a tela Novo Game. Se aqui aparecer correto, o publicação está com os imagens certas.</p></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Depois de publicar</span><h2>Checklist manual rápido</h2></div><span class="status-pill">Importante</span></div><div class="asset-checklist-v810">${checklist}</div></section>
  </section>`;
}
