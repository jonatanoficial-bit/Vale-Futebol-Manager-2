...
export function renderMoraleCrisisRibbon(state={}){
 ...
 return `<section class="morale-crisis-ribbon- panel">
  <div><span class="tag">Moral avançada</span><h3>${safe(c.activePlayer)}</h3><p class="small">${safe(c.activeRole)} · crise ${pct(c.crisisIndex)} · ${safe(c.recommendedAction)}</p></div>
  <div class="morale-ribbon-kpis-"><span>Ambiente ${pct(c.roomTemperature)}</span><span>Moral ${pct(c.activeMorale)}</span><span>Confiança ${pct(c.activeTrust)}</span></div>
...
export function renderMoraleCrisisCenter(state={}){
 const snap = buildMoraleCrisisSnapshot(state);
 const c = snap.context;
 const groups = MORALE_GROUPS_V690.map(g=>`<article class="card morale-group- risk-${safe(g.risk)}"><div class="row space"><strong>${safe(g.title)}</strong><span>${safe(g.risk)}</span></div><p>${safe(g.trigger)}</p><div class="meter"><span style="width:${pct(g.mood)}"></span></div><small>Humor ${pct(g.mood)} · influência ${pct(g.influence)}</small></article>`).join('');
 const triggers = CRISIS_TRIGGERS_V690.map(t=>`<article class="card crisis-trigger-"><div class="row space"><strong>${safe(t.title)}</strong><span>${pct(t.severity)}</span></div><p>${safe(t.source)} · ${safe(t.action)}</p><button class="secondary-btn mini" data-route="${safe(t.linkedRoute)}">Agir</button></article>`).join('');
 const cases = PLAYER_MORALE_CASES_V690.map(p=>`<div class="player-morale-row-"><div><strong>${safe(p.player)}</strong><small>${safe(p.role)} · ${safe(p.concern)}</small></div><div class="morale-mini-meter-"><span>${pct(p.morale)}</span><div class="meter"><span style="width:${pct(p.morale)}"></span></div></div><button class="secondary-btn mini" data-route="messages">Conversar</button></div>`).join('');
 const responses = COACH_RESPONSES_V690.map(r=>`<article class="card coach-response-"><strong>${safe(r.title)}</strong><p>${safe(r.effect)}</p><div class="response-impact-"><span>Moral ${r.morale>0?'+':''}${r.morale}</span><span>Diretoria ${r.board>0?'+':''}${r.board}</span><span>Imprensa ${r.press>0?'+':''}${r.press}</span></div><button class="secondary-btn mini" data-route="${safe(r.route)}">Aplicar</button></article>`).join('');
 ...
 return `<section class="morale-crisis-shell-">
  <section class="panel morale-crisis-hero-">
  ...
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Caso ativo</span><h2>${safe(c.activePlayer)}</h2></div><button class="main-btn mini" data-route="messages">Chamar conversa</button></div><div class="morale-focus-"><p><strong>Papel:</strong> ${safe(c.activeRole)}</p><p><strong>Preocupação:</strong> ${safe(c.activeConcern)}</p><p><strong>Pedido:</strong> ${safe(c.activeDemand)}</p><p><strong>Ação recomendada:</strong> ${safe(c.recommendedAction)}</p></div></article><article class="panel"><div class="row space"><div><span class="tag">Grupos</span><h2>Mapa do vestiário</h2></div><button class="secondary-btn mini" data-route="squad">Elenco</button></div><div class="morale-group-grid-">${groups}</div></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Gatilhos</span><h2>Crises em formação</h2></div><button class="secondary-btn mini" data-route="liveWorld">Jornal</button></div><div class="crisis-trigger-grid-">${triggers}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Jogadores</span><h2>Casos individuais</h2></div><button class="secondary-btn mini" data-route="contractRenewal">Promessas</button></div><div class="player-morale-list-">${cases}</div></article><article class="panel"><div class="row space"><div><span class="tag">Respostas</span><h2>Decisões do treinador</h2></div><button class="secondary-btn mini" data-route="pressConference">Coletiva</button></div><div class="coach-response-grid-">${responses}</div></article></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Mundo vivo</span><h2>Bastidores de hoje</h2></div><button class="secondary-btn mini" data-route="emotionalBoard">Diretoria</button></div><div class="news-list compact">${events}</div></article><article class="panel"><div class="row space"><div><span class="tag">Segurança</span><h2>Regras</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list morale-rules-">${rules}</ul></article></section>
