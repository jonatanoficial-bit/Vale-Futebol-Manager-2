...
export function buildReleaseCandidateSnapshot(state={}){
  ...
  if(completedSeasons === 0) warnings.push('Teste de temporada longa fica para v6.9.0; esta Beta é de primeira jornada mobile.');
...
export function renderReleaseCandidateCenter(state={}){
  ...
  const warnings = gate.warnings.length ? gate.warnings.map(w=>`<li>${w}</li>`).join('') : '<li>Nenhum aviso bloqueante para Beta Pública.</li>';
  return `<section class="release-candidate-v600 stack">
    <div class="panel standings-hero rc-hero-v600"><div><span class="tag">${snap.publicStatus.phase} · ${snap.publicStatus.label} · ${snap.version}</span><h1>Beta Pública Mobile-First</h1><p class="small">${snap.publicStatus.publicGoal} Esta etapa transforma a build v5.9.9 corrigida em uma versão organizada para teste real com jogadores, sem criar sistemas grandes novos e sem quebrar a base existente.</p></div><div class="hero-actions"><strong class="grade">${snap.score}%</strong><button class="main-btn" data-route="newGame">Testar início</button><button class="secondary-btn" data-route="match">Testar partida</button></div></div>
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Quality gate</span><h2>Checklist da Beta</h2></div><span class="status-pill">${gate.status}</span></div><div class="candidate-list">${checkpoints}</div></article><article class="panel"><div class="row space"><div><span class="tag">Matriz real</span><h2>Celular, tablet e PC</h2></div><button class="secondary-btn mini" data-action="safe-toast" data-message="Use a matriz para homologar a Beta em aparelhos reais.">Como usar</button></div><div class="news-list">${devices}</div><div class="training-note"><strong>Prioridade:</strong> ${snap.publicStatus.feedbackPriority.join(' · ')}</div></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Roteiro do testador</span><h2>Primeira jornada obrigatória</h2></div><span class="status-pill">10 passos</span></div><ol class="rc-flow-v600">${flow}</ol></article><article class="panel"><div class="row space"><div><span class="tag">Limites conhecidos</span><h2>O que observar sem pânico</h2></div><strong class="grade">Beta</strong></div><div class="candidate-list">${limits}</div></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Relatório rápido</span><h2>Estado do save e avisos</h2></div><button class="secondary-btn mini" data-route="saveCenter">Central de Save</button></div><div class="department-grid"><div class="department-kpi"><span>Técnico</span><strong>${snap.saveSignals.managerName}</strong><small>perfil atual</small></div><div class="department-kpi"><span>Clube</span><strong>${snap.saveSignals.clubId}</strong><small>save atual</small></div><div class="department-kpi"><span>Partidas</span><strong>${snap.saveSignals.completedMatches}</strong><small>concluídas</small></div><div class="department-kpi"><span>Temporadas</span><strong>${snap.saveSignals.completedSeasons}</strong><small>longa duração depois</small></div></div><ul class="small-list">${warnings}</ul><p class="alert"><strong>Decisão:</strong> ${gate.ok ? 'Build autorizada para teste público controlado. Próxima fase deve focar jornada inicial cinematográfica e retenção.' : gate.errors.join(' · ')}</p></section>
  </section>`;
}
