...
export function renderMatchdayPremiumStrip(state={}, snapOverride=null){
 ...
 return `<section class="matchday-premium-strip- panel" aria-label="Matchday premium">
  <div class="matchday-premium-glow-"></div>
  <div class="matchday-premium-copy-">
  ...
  <div class="matchday-premium-kpis-">
  ...
  <div class="matchday-premium-actions-">
...
export function renderMatchdayPremiumCenter(state={}){
 ...
 const flow = MATCHDAY_PREMIUM_FLOW_V630.map((item, index)=>`<button class="card matchday-flow-card-" data-route="${item.route}"><span>${item.icon}</span><strong>${index+1}. ${item.title}</strong><small>${item.focus}</small></button>`).join('');
 const packs = MATCHDAY_PREMIUM_PACKS_V630.map(pack=>`<article class="panel matchday-pack-"><div class="row space"><div><span class="tag">${pack.id}</span><h3>${pack.title}</h3></div><strong>${pack.items.length}/4</strong></div><ul class="small-list">${pack.items.map(x=>`<li>${x}</li>`).join('')}</ul></article>`).join('');
 const rules = MATCHDAY_PREMIUM_DEVICE_RULES_V630.map(rule=>`<li>${rule}</li>`).join('');
 const drama = MATCHDAY_PREMIUM_DRAMA_EVENTS_V630.map(ev=>`<div class="fixture-row matchday-drama-row-"><div class="fixture-date"><strong>${ev.minute}'</strong><small>drama</small></div><div class="fixture-main"><strong>${ev.title}</strong><span>${ev.tone}</span></div><span class="status-pill">ativo</span></div>`).join('');
 const report = JSON.stringify(snap, null, 2);
 return `<section class="matchday-premium-center- stack">
  <div class="panel championship-hero matchday-hero-">
  ...
  <section class="panel"><div class="row space"><div><span class="tag">Fluxo do dia de jogo</span><h2>Do pré-jogo ao jornal</h2></div><button class="main-btn mini" data-route="match">Jogar agora</button></div><div class="matchday-flow-grid-">${flow}</div></section>
  <section class="grid desktop-4">${packs}</section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Drama programado</span><h2>Momentos de tensão</h2></div><button class="secondary-btn mini" data-route="liveWorld">Jornal</button></div><div class="fixture-list">${drama}</div></article><article class="panel"><div class="row space"><div><span class="tag">Celular</span><h2>Regras segurança</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list matchday-rules-">${rules}</ul></article></section>
