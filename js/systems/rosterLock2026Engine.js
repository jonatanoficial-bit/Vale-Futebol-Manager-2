...
export const ROSTER_LOCK_2026_VERSION = 'v5.9.1';
...
export function renderRosterLock2026Center(state={}){
  ...
  return `<section class="roster-lock-v590 stack"><div class="panel championship-hero"><div><span class="tag">${snap.dataPack} · ${snap.lockDate}</span><h1>Roster Lock 20/05/2026</h1><p class="small">Auditoria total dos elencos oficiais/referência. Esta tela trava o pacote de dados e impede Release Candidate se houver genéricos, campos inválidos ou clubes jogáveis incompletos.</p></div><div class="release-score"><strong>${snap.releaseCandidateReady?'OK':'LOCK'}</strong><small>${snap.qualityGate}</small></div></div><section class="grid desktop-6">${cards}</section><section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Quality gate</span><h2>Status da base 2026</h2></div><strong class="grade">${snap.releaseCandidateReady?'A+':'BLOQ'}</strong></div><p class="small">Total auditado: ${snap.totals.totalPlayers} atletas/convocáveis. Genéricos detectados: ${snap.totals.totalGeneric}. Issues críticas: ${snap.totals.totalIssues}.</p><div class="fixture-list"><div class="fixture-row completed-fixture"><div class="fixture-date"><strong>A</strong><small>20/20</small></div><div class="fixture-main"><strong>Brasileirão Série A</strong><span>Elencos bloqueados sem genéricos</span></div><span class="status-pill">${snap.groups.serieA.status}</span></div><div class="fixture-row completed-fixture"><div class="fixture-date"><strong>B</strong><small>20/20</small></div><div class="fixture-main"><strong>Brasileirão Série B</strong><span>Elencos bloqueados sem genéricos</span></div><span class="status-pill">${snap.groups.serieB.status}</span></div><div class="fixture-row completed-fixture"><div class="fixture-date"><strong>🌎</strong><small>prioridade</small></div><div class="fixture-main"><strong>Pacotes internacionais prioritários</strong><span>CONMEBOL, elite global e seleções</span></div><span class="status-pill">${snap.groups.world.status}</span></div></div></article><article class="panel"><div class="row space"><div><span class="tag">Relatório JSON</span><h2>Snapshot técnico</h2></div><button class="secondary-btn mini" data-route="dataPack2026">Data Pack</button></div><textarea class="code-box" readonly>${report}</textarea></article></section><section class="panel"><div class="row space"><div><span class="tag">Amostra auditada</span><h2>Clubes e seleções</h2></div><span class="status-pill">${snap.qualityGate}</span></div><div class="table-scroll"><table class="table"><thead><tr><th>Pacote</th><th>Jogadores</th><th>Genéricos</th><th>Issues</th><th>Status</th></tr></thead><tbody>${sampleRows}</tbody></table></div></section></section>`;
}
