...
export function renderStadiumClimateStrip(state={}){
 ...
 return `<section class="stadium-climate--strip panel">
  <div><span class="tag">Clima e gramado</span><h3>${safe(c.headline)}</h3><p class="small">${safe(c.stadiumName)} · tempo ${pct(c.matchImpact.tempo)} · risco ${pct(c.matchImpact.injuryRisk)} · pressão ${pct(c.matchImpact.crowdPressure)}</p></div>
...
export function renderStadiumClimateCenter(state={}){
 const snap = buildStadiumClimateSnapshot(state);
 const c = snap.context;
 const climateCards = CLIMATE_PRESETS_V730.map(item => `<article class="card climate-card- ${item.id===c.selectedClimate.id?'active':''}"><div class="row space"><div><span class="tag">${safe(item.icon)} ${safe(item.title)}</span><h3>${safe(item.note)}</h3></div><span class="status-pill">${item.temperature}°C</span></div><div class="climate-bars-"><div><span>Chuva</span><b style="width:${pct(item.rain)}"></b><em>${pct(item.rain)}</em></div><div><span>Vento</span><b style="width:${pct(item.wind*2)}"></b><em>${item.wind} km/h</em></div><div><span>Passe</span><b style="width:${pct(60+item.passing)}"></b><em>${item.passing>0?'+':''}${item.passing}</em></div></div><button class="secondary-btn mini" data-action="set-ui" data-ui-key="stadiumClimateId" data-ui-value="${safe(item.id)}">Usar clima</button></article>`).join('');
 const pitchCards = PITCH_SURFACE_PROFILES_V730.map(item => `<article class="card pitch-card- ${item.id===c.selectedPitch.id?'active':''}"><div class="row space"><div><span class="tag">${safe(item.icon)} ${safe(item.title)}</span><h3>${safe(item.style)}</h3></div><span class="status-pill">risco ${pct(item.injuryRisk)}</span></div><div class="grid compact-stats"><div class="stat-line"><span>Tração</span><strong>${pct(item.traction)}</strong></div><div class="stat-line"><span>Drenagem</span><strong>${pct(item.drainage)}</strong></div><div class="stat-line"><span>Velocidade</span><strong>${pct(item.speed)}</strong></div><div class="stat-line"><span>Quique</span><strong>${pct(item.bounce)}</strong></div></div><button class="secondary-btn mini" data-action="set-ui" data-ui-key="stadiumPitchId" data-ui-value="${safe(item.id)}">Usar gramado</button></article>`).join('');
 const stadiumCards = STADIUM_CONTEXTS_V730.map(item => `<article class="card stadium-context- ${item.id===c.selectedStadium.id?'active':''}"><div class="row space"><div><span class="tag">${safe(item.icon)} ${safe(item.title)}</span><h3>${safe(item.mood)}</h3></div><span class="status-pill">pressão ${pct(item.pressure)}</span></div><div class="stat-line"><span>Receita</span><strong>${item.revenue>0?'+':''}${item.revenue}%</strong></div><div class="stat-line"><span>Intimidação</span><strong>${pct(item.intimidation)}</strong></div><button class="secondary-btn mini" data-action="set-ui" data-ui-key="stadiumContextId" data-ui-value="${safe(item.id)}">Usar contexto</button></article>`).join('');
 ...
 return `<section class="stadium-climate-shell-">
  ...
  <section class="panel"><div class="row space"><div><span class="tag">Clima</span><h2>Escolha o cenário</h2></div><span class="status-pill">offline seguro</span></div><div class="climate-grid-">${climateCards}</div></section>
  <section class="panel"><div class="row space"><div><span class="tag">Gramado</span><h2>Física do campo</h2></div><span class="status-pill">dinâmico</span></div><div class="climate-grid-">${pitchCards}</div></section>
  <section class="panel"><div class="row space"><div><span class="tag">Estádio</span><h2>Pressão e renda</h2></div><span class="status-pill">contexto</span></div><div class="climate-grid-">${stadiumCards}</div></section>

  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Regras</span><h2>Como afeta o jogo</h2></div><span class="status-pill">tático</span></div><div class="news-list compact">${rules}</div></article><article class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Validação </h2></div><span class="status-pill">segurança</span></div><textarea class="code-box" readonly>${safe(report)}</textarea></article></section>
 </section>`;
}
