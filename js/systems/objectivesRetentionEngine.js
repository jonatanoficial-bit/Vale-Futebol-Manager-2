...
export function renderObjectivesRetentionRibbon(state={}, snapOverride=null){
 ...
 return `<section class="objectives-ribbon- panel" aria-label="Objetivos e retenção">
  <div class="objectives-orb-"></div>
  <div class="objectives-ribbon-copy-"><span class="tag">${OBJECTIVES_RETENTION_VERSION} · objetivos vivos</span><h2>${safe(c.nextAction)}</h2><p>${safe(c.club)} · ${safe(c.league)} · ${safe(c.tierLabel)}</p></div>
  <div class="objectives-ribbon-kpis-"><div><span>Dia</span><strong>${pct(c.dailyProgress)}</strong></div><div><span>Retenção</span><strong>${pct(c.retentionScore)}</strong></div><div><span>Pontos</span><strong>${c.points}</strong></div></div>
  <div class="objectives-ribbon-actions-"><button class="main-btn mini" data-route="objectivesHub">Ver metas</button><button class="secondary-btn mini" data-route="careerTutorial">Missões</button></div>
 </section>`;
}
...
export function renderObjectivesRetentionCenter(state={}){
 ...
 const c = snap.context;
 const daily = DAILY_OBJECTIVES_V650.map(o=>`<article class="card objective-card-"><div class="row space"><span class="objective-icon-">${o.icon}</span><strong>${o.xp} XP</strong></div><h3>${o.title}</h3><p>${o.note}</p><div class="meter"><span style="width:${clamp(o.progress)}%"></span></div><div class="row space"><small>${o.reward}</small><button class="secondary-btn mini" data-route="${o.route}">abrir</button></div></article>`).join('');
 const season = SEASON_OBJECTIVES_V650.map(o=>`<div class="fixture-row season-objective-"><div class="fixture-date"><strong>${o.tier}</strong><small>meta</small></div><div class="fixture-main"><strong>${o.title}</strong><span>${o.reward}</span><small>Risco: ${o.risk}</small></div><button class="secondary-btn mini" data-route="${o.route}">ver</button></div>`).join('');
 const achievements = ACHIEVEMENTS_V650.map(a=>`<article class="panel achievement-card-"><div class="row space"><span class="achievement-icon-">${a.icon}</span><span class="tag">${a.rarity}</span></div><h3>${a.title}</h3><p class="small">${a.condition}</p><strong>${a.points} pts</strong></article>`).join('');
 const loops = RETENTION_LOOPS_V650.map((l,i)=>`<article class="card retention-loop-"><span class="tag">loop ${i+1}</span><h3>${l.title}</h3><p>${l.goal}</p><small>${l.feedback}</small></article>`).join('');
 const tiers = REWARD_TIERS_V650.map(t=>`<div class="tier-row- ${t.tier===c.tier?'active-tier-':''}"><strong>${t.tier}</strong><span>${t.min}-${t.max} pts · ${t.label}</span><small>${t.benefit}</small></div>`).join('');
 const rules = RETENTION_MOBILE_RULES_V650.map(rule=>`<li>${rule}</li>`).join('');
 const report = JSON.stringify(snap, null, 2);
 return `<section class="objectives-center- stack">
  <div class="panel championship-hero objectives-hero-"><div><span class="tag">${OBJECTIVES_RETENTION_VERSION} · ${OBJECTIVES_RETENTION_STATUS_V650.status}</span><h1>Objetivos, Conquistas e Retenção</h1><p class="small">Esta fase transforma a carreira em ciclo diário: sempre existe uma meta clara, uma recompensa visível e um próximo passo para o jogador continuar.</p></div><div class="release-score"><strong>${pct(c.retentionScore)}</strong><small>${safe(c.tier)}</small></div></div>
  ${renderObjectivesRetentionRibbon(state, snap)}
  <section class="panel"><div class="row space"><div><span class="tag">Rotina diária</span><h2>Missões rápidas de hoje</h2></div><button class="main-btn mini" data-route="matchdayPremium">Jogar dia</button></div><div class="objectives-grid-">${daily}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Temporada</span><h2>Metas grandes</h2></div><button class="secondary-btn mini" data-route="seasonCenter">Temporada</button></div><div class="fixture-list">${season}</div></article><article class="panel"><div class="row space"><div><span class="tag">Nível de retenção</span><h2>Recompensas</h2></div><strong>${c.points} pts</strong></div><div class="tier-list-">${tiers}</div></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Conquistas</span><h2>Troféus internos</h2></div><button class="secondary-btn mini" data-route="managerProgression">XP técnico</button></div><div class="achievements-grid-">${achievements}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Loop comercial</span><h2>Por que o jogador volta</h2></div><span class="status-pill">mobile-first</span></div><div class="retention-loop-grid-">${loops}</div></article><article class="panel"><div class="row space"><div><span class="tag">Segurança</span><h2>Regras mobile</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list objectives-rules-">${rules}</ul></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Validação da fase</h2></div><span class="status-pill">${snap.status}</span></div><textarea class="code-box" readonly>${report}</textarea></section>
 </section>`;
}
