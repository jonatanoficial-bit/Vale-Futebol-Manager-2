...
export function renderSoundAmbienceStrip(state={}){
 ...
 return `<section class="sound--strip panel">
  <div><span class="tag">Sons e torcida</span><h3>${safe(c.recommendedTitle)}</h3><p class="small">${safe(c.recommendedMood)} · volume seguro ${pct(c.recommendedVolume)} · ${c.webAudioAvailable ? 'WebAudio pronto' : 'modo silencioso seguro'}</p></div>
  <div class="sound--actions"><button class="main-btn mini" data-action="sound-ambience-play" data-preset="${safe(c.recommendedPreset)}">Ativar clima</button><button class="secondary-btn mini" data-action="sound-ambience-stop">Parar som</button><button class="secondary-btn mini" data-route="soundAmbience">Central</button></div>
 </section>`;
}
...
export function renderSoundAmbienceCenter(state={}){
 const snap = buildSoundAmbienceSnapshot(state);
 const c = snap.context;
 const presets = AMBIENCE_PRESETS_V710.map(p=>`<article class="card sound-preset- ${p.id===c.recommendedPreset?'recommended':''}"><div class="row space"><strong>${safe(p.title)}</strong><span>${pct(p.volume)}</span></div><p>${safe(p.description)}</p><div class="sound-bars-"><span style="height:${pct(p.crowd)}"></span><span style="height:${pct(p.tension)}"></span><span style="height:${pct(p.whistle)}"></span><span style="height:${pct(p.tempo)}"></span></div><small>${safe(p.mood)}</small><div class="row gap"><button class="main-btn mini" data-action="sound-ambience-play" data-preset="${safe(p.id)}">Tocar</button><button class="secondary-btn mini" data-route="${safe(p.route)}">Usar no contexto</button></div></article>`).join('');
 ...
 return `<section class="sound-ambience-shell-">
  <section class="panel sound-hero-">
  ...
  <section class="panel"><div class="row space"><div><span class="tag">Presets</span><h2>Climas sonoros do jogo</h2></div><button class="secondary-btn mini" data-action="sound-ambience-stop">Silenciar</button></div><div class="sound-preset-grid-">${presets}</div></section>
  ...
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Acessibilidade</span><h2>Regras anti-incômodo</h2></div><button class="secondary-btn mini" data-route="settings">Configurações</button></div><ul class="small-list sound-rules-">${rules}</ul></article><article class="panel"><div class="row space"><div><span class="tag">Mixagem</span><h2>Regras comerciais</h2></div><span class="status-pill">sem arquivo pesado</span></div><div class="news-list compact">${mixRules}</div></article></section>
