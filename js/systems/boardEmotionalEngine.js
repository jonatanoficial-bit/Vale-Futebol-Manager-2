...
export function renderBoardEmotionalRibbon(state={}, snapOverride=null){
 ...
 return `<section class="board-emotional-ribbon- panel" aria-label="Diretoria viva">
  <div class="board-emotional-orb-"></div>
  <div class="board-emotional-copy-"><span class="tag">${BOARD_EMOTIONAL_VERSION} · diretoria viva</span><h2>${safe(c.nextAction)}</h2><p>${safe(c.club)} · ${safe(c.scenario)} · risco ${safe(c.risk)}</p></div>
  <div class="board-emotional-kpis-"><div><span>Confiança</span><strong>${pct(c.boardTrust)}</strong></div><div><span>Autonomia</span><strong>${pct(c.autonomy)}</strong></div><div><span>Pressão</span><strong>${pct(c.pressure)}</strong></div></div>
  <div class="board-emotional-actions-"><button class="main-btn mini" data-route="emotionalBoard">Diretoria</button><button class="secondary-btn mini" data-route="financeCenter">Finanças</button></div>
 </section>`;
}
...
export function renderBoardEmotionalCenter(state={}){
 ...
 const c = snap.context;
 const pillars = BOARD_PILLARS_V660.map(p=>`<article class="card board-pillar-"><div class="row space"><span class="board-icon-">${p.icon}</span><strong>${p.weight}%</strong></div><h3>${p.title}</h3><p>${p.note}</p><button class="secondary-btn mini" data-route="${p.route}">abrir</button></article>`).join('');
 const scenarios = BOARD_SCENARIOS_V660.map(s=>`<div class="fixture-row board-scenario- ${s.title===c.scenario?'active-board-scenario-':''}"><div class="fixture-date"><strong>${s.range}</strong><small>${s.risk}</small></div><div class="fixture-main"><strong>${s.title}</strong><span>Tom: ${s.mood}</span><small>${s.action}</small></div></div>`).join('');
 const promises = BOARD_PROMISES_V660.map(p=>`<article class="panel board-promise-"><div class="row space"><span class="tag">prazo: ${p.deadline}</span><strong>+${p.trustImpact}</strong></div><h3>${p.title}</h3><p class="small">${p.failure}</p><div class="row space"><small>Torcida +${p.fanImpact}</small><button class="secondary-btn mini" data-route="${p.route}">agir</button></div></article>`).join('');
 const meetings = BOARD_MEETING_TYPES_V660.map(m=>`<article class="card board-meeting-"><span class="tag">${m.tone}</span><h3>${m.title}</h3><p>${m.trigger}</p><small>${m.output}</small><button class="secondary-btn mini" data-route="${m.route}">preparar</button></article>`).join('');
 const rules = EMOTIONAL_FINANCE_RULES_V660.map(rule=>`<li>${rule}</li>`).join('');
 const report = JSON.stringify(snap, null, 2);
 return `<section class="board-emotional-center- stack">
  <div class="panel championship-hero board-emotional-hero-"><div><span class="tag">${BOARD_EMOTIONAL_VERSION} · ${BOARD_EMOTIONAL_STATUS_V660.status}</span><h1>Economia Emocional e Diretoria Viva</h1><p class="small">A diretoria deixa de ser número frio: resultados, caixa, promessas, torcida e vestiário agora formam uma leitura viva da sua permanência e autonomia.</p></div><div class="release-score"><strong>${pct(c.emotionalEconomyScore)}</strong><small>${safe(c.mood)}</small></div></div>
  ${renderBoardEmotionalRibbon(state, snap)}
  <section class="grid desktop-4 board-score-grid-"><div class="card kpi-card"><span>Economia emocional</span><strong>${pct(c.emotionalEconomyScore)}</strong><div class="meter"><span style="width:${pct(c.emotionalEconomyScore)}"></span></div></div><div class="card kpi-card"><span>Autonomia de mercado</span><strong>${pct(c.autonomy)}</strong><div class="meter"><span style="width:${pct(c.autonomy)}"></span></div></div><div class="card kpi-card"><span>Pressão no cargo</span><strong>${pct(c.pressure)}</strong><div class="meter"><span style="width:${pct(c.pressure)}"></span></div></div><div class="card kpi-card"><span>Próxima reunião</span><strong>${safe(c.nextMeeting)}</strong><small>${safe(c.nextAction)}</small></div></section>
  <section class="panel"><div class="row space"><div><span class="tag">Leitura da diretoria</span><h2>Pilares que mudam sua autonomia</h2></div><button class="main-btn mini" data-route="objectivesHub">Ver metas</button></div><div class="board-pillars-grid-">${pillars}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Cenários</span><h2>Humor institucional</h2></div><strong>${safe(c.scenario)}</strong></div><div class="fixture-list">${scenarios}</div></article><article class="panel"><div class="row space"><div><span class="tag">Promessas</span><h2>Discurso vira cobrança</h2></div><button class="secondary-btn mini" data-route="pressConference">Coletiva</button></div><div class="board-promises-grid-">${promises}</div></article></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Reuniões</span><h2>Diretoria em movimento</h2></div><button class="secondary-btn mini" data-route="messages">E-mails</button></div><div class="board-meetings-grid-">${meetings}</div></article><article class="panel"><div class="row space"><div><span class="tag">Segurança</span><h2>Regras</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list board-rules-">${rules}</ul></article></section>
