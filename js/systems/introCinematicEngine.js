...
export function buildIntroCinematicSnapshot(state={}){
 ...
 return {
  ...
  managerAvatar: manager.avatar || 'assets/avatars/manager--01.png',
  clubId: club.id,
  clubName: club.name,
...
function kpi(label,value,note=''){
 return `<div class="department-kpi cinematic-kpi-"><span>${label}</span><strong>${value}</strong><small>${note}</small></div>`;
}

export function renderIntroCinematicCenter(state={}){
 const snap = buildIntroCinematicSnapshot(state);
 const acts = snap.acts.map(act=>`<article class="panel intro-act-card- ${act.current ? 'current' : ''}">
  <div class="intro-act-head-"><span class="intro-act-icon-">${act.icon}</span><div><span class="tag">Ato ${act.order} · ${act.tone} · ${act.duration}</span><h2>${act.title}</h2></div></div>
  <p>${act.subtitle}</p>
 ...
 const flow = snap.flow.map(item=>`<li><button class="intro-flow-button-" data-route="${item.route}"><strong>${item.step}. ${item.action}</strong><span>${item.expected}</span></button></li>`).join('');
 ...
 return `<section class="intro-cinematic- stack">
  <div class="panel intro-hero-" style="--intro-bg:url('${snap.heroBackground}')">
   <div class="intro-hero-copy-"><span class="tag">${snap.phase} · ${INTRO_PUBLIC_STATUS_V610.label} · ${snap.version}</span><h1>Primeira jornada com cara de jogo comercial</h1><p>${INTRO_PUBLIC_STATUS_V610.goal}</p><p class="small">${INTRO_PUBLIC_STATUS_V610.promise}</p><div class="menu-actions"><button class="main-btn giant" data-route="newGame">Começar carreira</button><button class="secondary-btn" data-route="lobby">Pular para lobby</button><button class="secondary-btn" data-route="match">Teste rápido da partida</button></div></div>
   <div class="intro-manager-contract-">${safeImg(snap.managerAvatar,'avatar','Manager','manager-pic')}<div><span class="small">Contrato inicial</span><strong>${esc(snap.managerName)}</strong><p>${safeImg(clubLogo(snap.clubId),'club',snap.clubName,'inline-club')} ${snap.clubName} · ${snap.clubLeague}</p></div></div>
  </div>
  ...
  <section class="panel"><div class="row space"><div><span class="tag">Abertura guiada</span><h2>Arco emocional do primeiro save</h2></div><button class="secondary-btn mini" data-route="releaseCandidate">Ver Primeiros passos</button></div><div class="intro-act-grid-">${acts}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Roteiro de 20 minutos</span><h2>Do menu ao primeiro pós-jogo</h2></div><span class="status-pill">Mobile-first</span></div><ol class="intro-flow-">${flow}</ol></article><article class="panel"><div class="row space"><div><span class="tag">Retenção</span><h2>Ganchos comerciais</h2></div><strong class="grade">${snap.score}%</strong></div><div class="news-list compact">${hooks}</div></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Segurança mobile</span><h2>Regras da jornada cinematográfica</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação Mobile</button></div><ul class="small-list intro-rules-">${rules}</ul><p class="alert"><strong>Decisão:</strong> a Beta agora ganha uma entrada mais emocional. Próxima fase pode aprofundar lobby vivo, jornal esportivo e mensagens de bastidor.</p></section>
 </section>`;
}
