...
export function buildRealAudioPackSnapshot(state={}){
  ...
  return {
    ...
    manifestRoot: REAL_AUDIO_ASSET_ROOT_V720,
...
export function renderRealAudioPackStrip(state={}){
  ...
  return `<section class="real-audio-v720-strip panel">
    <div><span class="tag">v7.2 · Efeitos reais opcionais</span><h3>${safe(c.recommendedTitle)}</h3><p class="small">${safe(c.recommendedCategory)} · ${safe(c.scanLabel)} · fallback ${safe(c.recommendedFallback)}</p></div>
    <div class="real-audio-v720-actions"><button class="main-btn mini" data-action="real-audio-play" data-cue="${safe(c.recommendedTrack)}">Tocar real/fallback</button><button class="secondary-btn mini" data-action="real-audio-stop">Parar</button><button class="secondary-btn mini" data-route="realAudioPack">Pacote</button></div>
  </section>`;
}
...
export function renderRealAudioPackCenter(state={}){
  ...
  const tracks = REAL_AUDIO_TRACKS_V720.map(track => {
    ...
    return `<article class="card real-track-v720 ${detected?.found?'found':'fallback'}"><div class="row space"><div><span class="tag">${safe(track.category)} · ${track.loop?'loop':'efeito curto'}</span><h3>${safe(track.title)}</h3></div><span class="status-pill">${safe(badge)}</span></div><p>${safe(track.mood)} · volume ${pct(track.volume)} · ${safe(track.durationHint)}</p><div class="real-paths-v720">${paths}</div><div class="row gap"><button class="main-btn mini" data-action="real-audio-play" data-cue="${safe(track.id)}">Tocar</button><button class="secondary-btn mini" data-route="${safe((track.contextRoutes||['match'])[0])}">Contexto</button></div></article>`;
  }).join('');
  ...
  const manifest = JSON.stringify({ version:REAL_AUDIO_PACK_VERSION, root:REAL_AUDIO_ASSET_ROOT_V720, tracks:REAL_AUDIO_TRACKS_V720.map(t=>({id:t.id,title:t.title,loop:t.loop,volume:t.volume,paths:t.paths,fallbackPreset:t.fallbackPreset})) }, null, 2);
  const report = JSON.stringify(snap, null, 2);
  return `<section class="real-audio-shell-v720">
    <section class="panel real-audio-hero-v720"><div><span class="tag">${safe(REAL_AUDIO_PACK_STATUS_V720.phase)}</span><h1>${safe(REAL_AUDIO_PACK_STATUS_V720.label)}</h1><p class="subtitle">Agora o jogo aceita MP3/WAV/OGG reais de torcida, apito, estádio, trilha e interface. Se o arquivo ainda não existir, o fallback WebAudio v7.1.0 entra automaticamente sem quebrar a partida.</p></div><div class="row gap"><button class="main-btn" data-action="real-audio-play" data-cue="${safe(c.recommendedTrack)}">Tocar recomendado</button><button class="secondary-btn" data-action="real-audio-scan">Verificar arquivos</button><button class="secondary-btn danger" data-action="real-audio-stop">Parar tudo</button></div></section>
    ...
    <section class="panel"><div class="row space"><div><span class="tag">Manifest de áudio</span><h2>Arquivos reais aceitos</h2></div><button class="secondary-btn mini" data-action="real-audio-scan">Escanear</button></div><div class="real-track-grid-v720">${tracks}</div></section>

    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Formatos</span><h2>Compatibilidade mobile</h2></div><span class="status-pill">MP3 recomendado</span></div>${formats}</article><article class="panel"><div class="row space"><div><span class="tag">Importação</span><h2>Como inserir seus sons</h2></div><button class="secondary-btn mini" data-route="settings">Configurações</button></div><ul class="real-steps-v720">${steps}</ul></article></section>

    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Anti-quebra</span><h2>Regras de segurança</h2></div><span class="status-pill">fallback ativo</span></div><div class="news-list compact">${safety}</div></article><article class="panel"><div class="row space"><div><span class="tag">Manifest JSON</span><h2>Mapa técnico</h2></div><span class="status-pill">assets/audio</span></div><textarea class="code-box" readonly>${safe(manifest)}</textarea></article></section>

    <section class="panel"><div class="row space"><div><span class="tag">Snapshot técnico</span><h2>Validação v7.2.0</h2></div><span class="status-pill">${safe(snap.status)}</span></div><textarea class="code-box" readonly>${safe(report)}</textarea></section>
  </section>`;
}
