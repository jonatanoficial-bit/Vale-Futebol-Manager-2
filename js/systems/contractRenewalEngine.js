...
export function renderContractRenewalRibbon(state={}){
 ...
 return `<section class="contract-renewal-ribbon- panel">
  <div><span class="tag">Contratos profundos</span><h3>${safe(c.activePlayer)}</h3><p class="small">${safe(c.activeRole)} · ${c.monthsLeft} meses · ${safe(c.recommendedAction)}</p></div>
  <div class="contract-ribbon-kpis-"><span>Pressão ${pct(c.renewalPressure)}</span><span>Diretoria ${pct(c.boardComfort)}</span><span>Impacto salarial +${c.projectedSalaryImpact}%</span></div>
...
export function renderContractRenewalCenter(state={}){
 ...
 const c = snap.context;
 const target = activeTarget(state);
 const targets = RENEWAL_TARGETS_V680.map(t => `<article class="card renewal-target- ${t.id===target.id?'active':''}"><div class="row space"><strong>${safe(t.player)}</strong><span>${t.monthsLeft}m</span></div><p>${safe(t.role)} · ${safe(t.agent)}</p><div class="meter"><span style="width:${pct(t.morale)}"></span></div><small>Risco: ${safe(t.risk)} · salário ${money(t.currentSalary)} → ${money(t.requestedSalary)}</small><button class="secondary-btn mini" data-route="${t.route}">Contexto</button></article>`).join('');
 const packages = CONTRACT_PACKAGES_V680.map(p => `<article class="card contract-package-"><span class="contract-icon-">${p.icon}</span><strong>${safe(p.title)}</strong><p>Salário +${p.salaryBoost}% · luvas ${money(p.signingFee)}</p><small>${safe(p.bonus)}</small><div class="contract-impact-row-"><span>Moral +${p.moraleImpact}</span><span>Diretoria ${p.boardImpact>0?'+':''}${p.boardImpact}</span></div><button class="secondary-btn mini" data-route="${p.route}">Avaliar</button></article>`).join('');
 const promises = CONTRACT_PROMISES_V680.map(p => `<div class="promise-row-"><div><strong>${safe(p.title)}</strong><small>${safe(p.area)} · ${safe(p.consequence)}</small></div><button class="secondary-btn mini" data-route="objectivesHub">Vincular</button></div>`).join('');
 const clauses = CONTRACT_CLAUSES_V680.map(cl => `<div class="clause-chip-"><strong>${safe(cl.title)}</strong><span>${safe(cl.risk)}</span></div>`).join('');
 ...
 return `<section class="contract-renewal-shell-">
  <section class="panel contract-renewal-hero-">
  ...
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Alvo ativo</span><h2>${safe(c.activePlayer)}</h2></div><button class="main-btn mini" data-route="matchdayPremium">Dar minutos</button></div><div class="renewal-focus-"><p><strong>Papel:</strong> ${safe(c.activeRole)}</p><p><strong>Empresário:</strong> ${safe(c.agent)}</p><p><strong>Moral:</strong> ${pct(c.morale)}</p><p><strong>Ação recomendada:</strong> ${safe(c.recommendedAction)}</p></div></article><article class="panel"><div class="row space"><div><span class="tag">Lista crítica</span><h2>Renovações prioritárias</h2></div><button class="secondary-btn mini" data-route="squadAI">Vestiário</button></div><div class="renewal-target-grid-">${targets}</div></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Pacotes</span><h2>Modelos de proposta</h2></div><button class="secondary-btn mini" data-route="financeCenter">Finanças</button></div><div class="contract-package-grid-">${packages}</div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Promessas</span><h2>O que fica registrado</h2></div><button class="secondary-btn mini" data-route="objectivesHub">Objetivos</button></div><div class="promise-list-">${promises}</div></article><article class="panel"><div class="row space"><div><span class="tag">Cláusulas</span><h2>Detalhes que mudam o jogo</h2></div><button class="secondary-btn mini" data-route="agentMarket">Negociar</button></div><div class="clause-grid-">${clauses}</div></article></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Mundo vivo</span><h2>Eventos de contrato</h2></div><button class="secondary-btn mini" data-route="liveWorld">Jornal</button></div><div class="news-list compact">${events}</div></article><article class="panel"><div class="row space"><div><span class="tag">Segurança</span><h2>Regras</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list renewal-rules-">${rules}</ul></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Validação </h2></div><span class="status-pill">${snap.status}</span></div><textarea class="code-box" readonly>${safe(report)}</textarea></section>
 </section>`;
}
