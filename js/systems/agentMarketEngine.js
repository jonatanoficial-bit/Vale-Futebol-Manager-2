...
export function renderAgentMarketRibbon(state={}, snapOverride=null){
 ...
 return `<section class="agent-market-ribbon- panel" aria-label="Negociações vivas">
  <div class="agent-market-orb-"></div>
  <div class="agent-market-copy-"><span class="tag">${AGENT_MARKET_VERSION} · empresários</span><h2>${safe(c.activePlayer)}</h2><p>${safe(c.activeAgent)} · rival: ${safe(c.activeRival)} · risco ${pct(c.dealRisk)}</p></div>
  <div class="agent-market-kpis-"><div><span>Chance</span><strong>${pct(c.closeChance)}</strong></div><div><span>Autonomia</span><strong>${pct(c.boardAutonomy)}</strong></div><div><span>Pressão</span><strong>${pct(c.pressure)}</strong></div></div>
  <div class="agent-market-actions-"><button class="main-btn mini" data-route="agentMarket">Negociar</button><button class="secondary-btn mini" data-route="transfers">Mercado</button></div>
 </section>`;
}
...
export function renderAgentMarketCenter(state={}){
 ...
 const c = snap.context;
 const agents = AGENT_PROFILES_V670.map(a=>`<article class="card agent-card-"><div class="row space"><span class="agent-icon-">${a.icon}</span><strong>${a.trust}%</strong></div><h3>${a.name}</h3><p>${a.note}</p><div class="meter"><span style="width:${pct(a.pressure)}"></span></div><small>pressão ${a.pressure}% · tom ${a.temperament}</small><button class="secondary-btn mini" data-route="${a.route}">agir</button></article>`).join('');
 const stages = NEGOTIATION_STAGES_V670.map((s,i)=>`<div class="fixture-row negotiation-stage- ${i===1?'active-agent-stage-':''}"><div class="fixture-date"><strong>${s.icon}</strong><small>${s.risk}</small></div><div class="fixture-main"><strong>${s.title}</strong><span>${s.action}</span></div><button class="secondary-btn mini" data-route="${s.route}">abrir</button></div>`).join('');
 const cases = LIVE_NEGOTIATION_CASES_V670.map(deal=>{ const ag=agentById(deal.agent); return `<article class="panel live-deal- ${deal.player===c.activePlayer?'active-live-deal-':''}"><div class="row space"><span class="tag">${ag.name}</span><strong>${money(deal.value)}</strong></div><h3>${deal.player}</h3><p class="small">${deal.club} · ${deal.mood}</p><div class="stat-line"><span>Pedido</span><strong>${deal.demand}</strong></div><div class="stat-line"><span>Concorrência</span><strong>${deal.rival}</strong></div><button class="main-btn mini" data-route="${deal.route}">responder</button></article>`; }).join('');
 const levers = CONTRACT_LEVERS_V670.map(l=>`<article class="card contract-lever-"><h3>${l.title}</h3><p>${l.effect}</p></article>`).join('');
