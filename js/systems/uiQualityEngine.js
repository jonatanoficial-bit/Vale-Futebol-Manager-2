...
export function installViewportGuards(){
 ...
 if(!document.querySelector('style[data-vfm-aaa-runtime="viewport-guards"]')){
  ...
  style.textContent = `
   .vfm-aaa- [data-route], .vfm-aaa- [data-action]{ cursor:pointer; }
   .vfm-aaa- .is-current-route{ border-color:rgba(255,216,106,.48)!important; background:rgba(255,216,106,.13)!important; }
   .vfm-aaa- .screen{ contain:paint; }
   .vfm-aaa- .panel, .vfm-aaa- .card{ overflow-wrap:anywhere; }
  `;
...
export function markCurrentRoute(){
 try{
  const state = JSON.parse(localStorage.getItem('vfm-save-') || '{}');
...
export function renderUiAaaCenter(state={}){
 ...
 const checks = [
  ...
  ['Segurança visual', 'OK', 'Overflow protegido, fallback de fundos e build badge sempre visível.'],
 ...
 return `<section class="aaa-shell">
  <div class="panel aaa-hero">
   <div><span class="tag">${snapshot.version} · UI AAA</span><h1>Polimento comercial</h1><p class="small">Camada visual premium para mobile e desktop, mantendo o jogo leve, sem dependências pagas e com proteções segurança.</p></div>
  ...
  <section class="aaa-demo-grid">
   <article class="panel"><div class="row space"><div><span class="tag">Quality Gate</span><h2>Checklist visual </h2></div><strong class="grade">6/6</strong></div><div class="aaa-check-grid">${checks.map(([name,status,desc])=>`<div class="aaa-check ok"><span class="aaa-pill">${status}</span><strong>${name}</strong><small>${desc}</small></div>`).join('')}</div></article>
