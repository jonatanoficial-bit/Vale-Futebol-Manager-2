...
function wire(scope){
  ...
  scope.querySelectorAll('[data-action="training-apply-session"]').forEach(btn => btn.addEventListener('click', () => { applyWeeklyTrainingSession(btn.dataset.session || 'tactical'); render(); }));
  scope.querySelectorAll('[data-action="training-set-preset"]').forEach(btn => btn.addEventListener('click', () => { setTrainingWeeklyPreset(btn.dataset.preset || 'balanced'); render(); }));
  ...
  scope.querySelectorAll('[data-action="scout-generate-report"]').forEach(btn => btn.addEventListener('click', () => { generateScoutReport(); render(); }));
  scope.querySelectorAll('[data-action="scout-assign-region"]').forEach(btn => btn.addEventListener('click', () => { assignScoutRegion(btn.dataset.region || getState().ui?.scoutingRegion || 'br-sudeste'); render(); }));
  scope.querySelectorAll('[data-action="scout-focus"]').forEach(sel => sel.addEventListener('change', () => { setScoutFocus(sel.value || 'rotation'); render(); }));
  scope.querySelectorAll('[data-action="scout-wishlist-add"]').forEach(btn => btn.addEventListener('click', () => { addScoutWishlist(btn.dataset.report || ''); render(); }));
  scope.querySelectorAll('[data-action="scout-wishlist-remove"]').forEach(btn => btn.addEventListener('click', () => { removeScoutWishlist(btn.dataset.report || ''); render(); }));
  scope.querySelectorAll('[data-action="staff-hire"]').forEach(btn => btn.addEventListener('click', () => { hireStaffMember(btn.dataset.candidate || ''); render(); }));
  scope.querySelectorAll('[data-action="staff-focus"]').forEach(btn => btn.addEventListener('click', () => { setStaffFocus(btn.dataset.focus || 'balanced'); render(); }));
  scope.querySelectorAll('[data-action="staff-meeting"]').forEach(btn => btn.addEventListener('click', () => { runStaffMeeting(); render(); }));
  scope.querySelectorAll('[data-action="finance-sign-sponsor"]').forEach(btn => btn.addEventListener('click', () => { signFinanceSponsor(btn.dataset.sponsor || ''); render(); }));
  scope.querySelectorAll('[data-action="finance-ticket-policy"]').forEach(btn => btn.addEventListener('click', () => { setFinanceTicketPolicy(btn.dataset.policy || 'balanced'); render(); }));
  scope.querySelectorAll('[data-action="finance-matchday"]').forEach(btn => btn.addEventListener('click', () => { simulateFinanceMatchday(); render(); }));
  scope.querySelectorAll('[data-action="finance-prize"]').forEach(btn => btn.addEventListener('click', () => { applyFinancePrize(btn.dataset.prize || 'league-win'); render(); }));
  scope.querySelectorAll('[data-action="finance-board-meeting"]').forEach(btn => btn.addEventListener('click', () => { runFinanceBoardMeeting(); render(); }));
