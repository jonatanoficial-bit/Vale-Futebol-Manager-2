...
function wire(scope){
  ...
  scope.querySelectorAll('[data-action="load-save-slot"]').forEach(btn => btn.addEventListener('click', () => { loadSaveSlot(btn.dataset.slot || 'principal'); go('lobby'); }));
  scope.querySelectorAll('[data-action="new-career-slot"]').forEach(btn => btn.addEventListener('click', () => { createNewCareerSlot(btn.dataset.slot || 'career-2'); go('newGame'); }));
