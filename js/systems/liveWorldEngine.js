...
function kpi(label,value,note=''){
  return `<div class="department-kpi live-world-kpi-v620"><span>${label}</span><strong>${value}</strong><small>${note}</small></div>`;
}
...
export function renderLiveWorldCenter(state={}){
  ...
  const headlines = snap.headlines.map((item,index)=>`<article class="live-headline-card-v620 ${index===0?'lead':''}">
    <div class="live-headline-icon-v620">${toneIcon(item.tone)}</div>
  ...
  const backstage = snap.backstage.map(item=>`<article class="news-item live-backstage-v620"><strong>${item.department}</strong><span>${item.signal}</span><small>${item.mood}</small><em>${item.risk}</em></article>`).join('');
  const sections = snap.sections.map(item=>`<div class="live-section-pill-v620"><span>${item.icon}</span><strong>${item.title}</strong><small>${item.description}</small></div>`).join('');
  ...
  return `<section class="live-world-v620 stack">
    <div class="panel live-world-hero-v620" style="--live-bg:url('${snap.heroBackground}')">
      ...
      <div class="live-club-pulse-v620">${safeImg(clubLogo(snap.clubId),'club',snap.clubName,'club-logo xl')}<div><span class="small">Pulso do dia</span><strong>${esc(snap.clubName)}</strong><p>${snap.league} · ${snap.stadium}</p><p class="small">Próximo clima: ${snap.nextTitle}</p></div></div>
    </div>
    <section class="grid desktop-4">${kpi('Status', snap.status, 'quality gate')}${kpi('Urgência', `${snap.score}%`, 'mundo vivo')}${kpi('Torcida', `${snap.fanMood}%`, 'humor')}${kpi('Diretoria', `${snap.boardTrust}%`, 'confiança')}</section>
    <section class="panel"><div class="row space"><div><span class="tag">Jornal esportivo</span><h2>Manchetes que puxam o jogador para agir</h2></div><button class="secondary-btn mini" data-route="messages">E-mails</button></div><div class="live-headline-grid-v620">${headlines}</div></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Bastidores</span><h2>O que acontece no clube hoje</h2></div><strong class="grade">Ao vivo</strong></div><div class="news-list">${backstage}</div></article><article class="panel"><div class="row space"><div><span class="tag">Editorias</span><h2>Estrutura do jornal</h2></div><span class="status-pill">${snap.sections.length} blocos</span></div><div class="live-section-grid-v620">${sections}</div></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Ritual diário</span><h2>Loop de retenção do lobby</h2></div><button class="main-btn mini" data-route="careerIntro">Jornada</button></div><ol class="rc-flow-v600 live-flow-v620">${hooks}</ol></article><article class="panel"><div class="row space"><div><span class="tag">Matriz mobile</span><h2>Garantias da fase</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Auditoria</button></div><div class="candidate-list">${devices}</div></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Anti-quebra</span><h2>Regras de segurança do mundo vivo</h2></div><button class="secondary-btn mini" data-route="releaseCandidate">Beta Pública</button></div><ul class="small-list live-rules-v620">${rules}</ul><p class="alert"><strong>Decisão:</strong> o lobby agora tem jornal, bastidores e motivo para o jogador abrir tática, tabela, coletiva e partida sem depender de internet ou vídeo pesado.</p></section>
  </section>`;
}
