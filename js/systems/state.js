...
const DEFAULT_MANAGER_AVATAR_V810 = 'assets/avatars/manager-v810-01.png';
function normalizeManagerAvatarPath(src=''){
  const value = String(src || '').trim() || DEFAULT_MANAGER_AVATAR_V810;
  const legacy = value.match(/^assets\/avatars\/manager-(?:v801-)?(\d{2})\.png$/);
  if(legacy) return `assets/avatars/manager-v810-${legacy[1]}.png`;
  const v810 = value.match(/^assets\/avatars\/manager-v810-(\d{2})\.png$/);
  if(v810) return value;
  return value.includes('assets/avatars/') ? value : DEFAULT_MANAGER_AVATAR_V810;
}

export const defaultState = () => ({
  route:'cover',
  manager:{ name:'Joao Victor', country:'br', avatar:DEFAULT_MANAGER_AVATAR_V810, reputation:82, mode:'career' },
...
function sanitizeManager(manager={}){
  ...
  return {
    ...
    avatar: normalizeManagerAvatarPath(manager.avatar || DEFAULT_MANAGER_AVATAR_V810),
...
function normalize(next){
  ...
  merged.ui.selectedAvatar = normalizeManagerAvatarPath(merged.ui.selectedAvatar || merged.manager.avatar || DEFAULT_MANAGER_AVATAR_V810);
