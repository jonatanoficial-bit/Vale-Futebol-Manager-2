...
export function ensureFinanceState(finance={}, state={}){
  ...
  return {
    ...
    ledger:Array.isArray(finance.ledger) ? finance.ledger.slice(-60) : ['Financeiro profundo ativado com patrocínio, bilheteria, folha e premiações.'],
...
export function renderFinanceRibbon(state={}){
  const snap = buildFinanceSnapshot(state);
  return `<section class="finance-ribbon-v790 panel"><div><span class="tag">Finanças</span><h3>${snap.health} · saldo mensal ${moneyBRL(snap.monthlyBalance)}</h3><p class="small">Caixa ${moneyBRL(snap.finance.cash)} · folha ${snap.wagePressure}% da receita · patrocínio ${snap.sponsorSatisfaction}% · estádio ${snap.stadium.occupation}%</p></div><div class="row gap"><button class="secondary-btn mini" data-route="financeCenter">Centro financeiro</button><button class="secondary-btn mini" data-action="finance-board-meeting">Reunião</button></div></section>`;
}
