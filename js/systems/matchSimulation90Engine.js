...
export function renderMatchSimulation90Strip(state={}){
 ...
 return `<section class="match-sim90-strip- panel">
  <div><span class="tag">Simulação 90 minutos</span><h3>${safe(c.activePhase)}</h3><p class="small">${safe(c.tacticalFocus)} · pressão ${pct(c.pressureIndex)}</p></div>
  <div class="sim90-strip-kpis-"><span>${safe(c.keyEvent)}</span><span>${safe(c.coachCommand)}</span></div>
...
export function renderMatchSimulation90Center(state={}){
 const snap = buildMatchSimulation90Snapshot(state);
 const c = snap.context;
 const phases = MATCH_PHASES_V700.map(p=>`<article class="card sim90-phase- ${p.title===c.activePhase?'active':''}"><div class="row space"><strong>${safe(p.title)}</strong><span>${pct(p.intensity)}</span></div><p>${safe(p.tactical)}</p><small>${safe(p.coachCue)}</small><div class="meter"><span style="width:${pct(p.intensity)}"></span></div></article>`).join('');
 const events = MOMENTUM_EVENTS_V700.map(e=>`<div class="sim90-event-dot- ${safe(e.type)}" style="left:${pct(e.x)};top:${pct(e.y)}"><span>${safe(e.minute)}'</span><small>${safe(e.title)}</small></div>`).join('');
 ...
 const reads = TACTICAL_READS_V700.map(r=>`<article class="card sim90-read-"><div class="row space"><strong>${safe(r.title)}</strong><span>risco ${pct(r.risk)}</span></div><p>${safe(r.advice)}</p><div class="response-impact-"><span>Risco ${pct(r.risk)}</span><span>Recompensa ${pct(r.reward)}</span></div><button class="secondary-btn mini" data-route="${safe(r.route)}">Aplicar leitura</button></article>`).join('');
 const commands = COACH_INTERVENTIONS_V700.map(i=>`<article class="card sim90-command-"><div class="row space"><strong>${safe(i.title)}</strong><span>${safe(i.minuteWindow)}</span></div><p>${safe(i.effect)}</p><div class="response-impact-"><span>Moral ${i.morale>0?'+':''}${i.morale}</span><span>Tática ${i.tactical>0?'+':''}${i.tactical}</span><span>Risco ${i.risk>0?'+':''}${i.risk}</span></div><button class="secondary-btn mini" data-route="${safe(i.route)}">Comandar</button></article>`).join('');
 ...
 return `<section class="match-sim90-shell-">
  <section class="panel sim90-hero-">
  ...
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Mapa 2D textual</span><h2>Momentos no campo</h2></div><button class="secondary-btn mini" data-route="match">Partida ao vivo</button></div><div class="sim90-pitch-"><div class="pitch-lines"><span class="center-circle"></span><span class="box left"></span><span class="box right"></span></div>${events}</div><div class="news-list compact">${eventRows}</div></article><article class="panel"><div class="row space"><div><span class="tag">Fases da partida</span><h2>Roteiro dos 90 minutos</h2></div><span class="status-pill">${safe(snap.status)}</span></div><div class="sim90-phase-grid-">${phases}</div></article></section>

  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Leitura tática</span><h2>O que o assistente enxerga</h2></div><button class="secondary-btn mini" data-route="formation">Tática</button></div><div class="sim90-read-grid-">${reads}</div></article><article class="panel"><div class="row space"><div><span class="tag">Decisão do treinador</span><h2>Microcomandos premium</h2></div><button class="secondary-btn mini" data-route="instructions">Instruções</button></div><div class="sim90-command-grid-">${commands}</div></article></section>

  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Narração premium</span><h2>Como a partida ganha vida</h2></div><button class="secondary-btn mini" data-route="liveWorld">Jornal</button></div><div class="news-list compact">${commentary}</div></article><article class="panel"><div class="row space"><div><span class="tag">Segurança</span><h2>Regras</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list sim90-rules-">${rules}</ul></article></section>

  <section class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Verificação</h2></div><span class="status-pill">${safe(snap.status)}</span></div><textarea class="code-box" readonly>${safe(report)}</textarea></section>
 </section>`;
}
