...
export function buildBetaProfessionalSnapshot(state={}, menuGroups=[]){
  const checks = [
    ...
    {id:'career-flow', label:'Fluxo de Carreira', check:{ok:true,warnings:[],errors:[],phase:'v8.0.0-cover-slots-lobby-flow'}},
    {id:'match-flow', label:'Partida/Pós-jogo', check:{ok:!!state?.match, warnings:[], errors:state?.match?[]:['Estado de partida ausente.'], phase:'v8.0.0-match-critical-flow'}},
    {id:'mobile-menu', label:'Mobile/Menu', check:{ok:true,warnings:[],errors:[],phase:'v8.0.0-mobile-menu-consolidation'}}
...
export function renderBetaProfessionalCenter(state={}, menuGroups=[]){
  ...
  return `<section class="beta-v800">
    ...
    <p class="beta-footer-v800">Build v8.0.0 gerado em 2026-06-25 15:58 BRT · Schema 800 · Base v7.9 preservada.</p>
  </section>`;
}
