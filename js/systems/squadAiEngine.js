...
export function renderSquadAiRibbon(state={}, snapOverride=null){
 ...
 return `<section class="squad-ai-ribbon- panel" aria-label="IA de elenco e vestiário">
  <div class="squad-ai-orb-"></div>
  <div class="squad-ai-ribbon-copy-"><span class="tag">${SQUAD_AI_VERSION} · IA de Vestiário</span><h2>${safe(c.club)}</h2><p>${safe(c.league)} · clima ${safe(c.dressingRoom)} · ${safe(c.lastResult)}</p></div>
  <div class="squad-ai-ribbon-kpis-"><div><span>Moral</span><strong>${pct(c.morale)}</strong></div><div><span>Liderança</span><strong>${pct(c.leadership)}</strong></div><div><span>Risco</span><strong>${pct(c.crisisRisk)}</strong></div></div>
  <div class="squad-ai-ribbon-actions-"><button class="main-btn mini" data-route="squadAI">${call}</button><button class="secondary-btn mini" data-route="squad">Elenco</button></div>
 </section>`;
}

export function renderSquadAiCenter(state={}){
 const snap = buildSquadAiSnapshot(state);
 const pillars = SQUAD_AI_PILLARS_V640.map((p,i)=>`<button class="card squad-ai-pillar-" data-route="${p.route}"><span>${p.icon}</span><strong>${i+1}. ${p.title}</strong><small>${p.focus}</small></button>`).join('');
 const profiles = SQUAD_AI_PLAYER_PROFILES_V640.map(p=>`<article class="panel squad-ai-profile-"><div class="row space"><div><span class="tag">${p.role}</span><h3>${p.mood}% moral</h3></div><strong>${p.risk}% risco</strong></div><div class="squad-ai-bars-"><label>Influência<span>${p.influence}%</span></label><div class="meter"><span style="width:${p.influence}%"></span></div><label>Risco<span>${p.risk}%</span></label><div class="meter danger-meter-"><span style="width:${p.risk}%"></span></div></div><p class="small">${p.note}</p></article>`).join('');
 const events = SQUAD_AI_EVENTS_V640.map(ev=>`<div class="fixture-row squad-ai-event- ${ev.severity==='alta'?'danger-row-':''}"><div class="fixture-date"><strong>${ev.severity}</strong><small>evento</small></div><div class="fixture-main"><strong>${ev.title}</strong><span>${ev.action}</span></div><button class="secondary-btn mini" data-route="${ev.route}">agir</button></div>`).join('');
 const decisions = SQUAD_AI_DECISIONS_V640.map(d=>`<article class="card squad-ai-decision-"><strong>${d.title}</strong><span>${d.impact}</span><small>${d.bestFor}</small></article>`).join('');
 const rules = SQUAD_AI_MOBILE_RULES_V640.map(rule=>`<li>${rule}</li>`).join('');
 const report = JSON.stringify(snap, null, 2);
 return `<section class="squad-ai-center- stack">
  <div class="panel championship-hero squad-ai-hero-"><div><span class="tag">${SQUAD_AI_VERSION} · ${SQUAD_AI_STATUS_V640.status}</span><h1>IA de Elenco e Vestiário</h1><p class="small">A fase adiciona leitura de moral, liderança, crise, rotação e reação do grupo para transformar elenco em sistema vivo.</p></div><div class="release-score"><strong>${pct(snap.context.morale)}</strong><small>${snap.context.dressingRoom}</small></div></div>
  ${renderSquadAiRibbon(state, snap)}
  <section class="panel"><div class="row space"><div><span class="tag">Pilares da IA</span><h2>Como o elenco reage</h2></div><button class="main-btn mini" data-route="squad">Ver elenco</button></div><div class="squad-ai-grid-">${pillars}</div></section>
  <section class="grid desktop-5">${profiles}</section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Eventos de vestiário</span><h2>Alertas vivos</h2></div><button class="secondary-btn mini" data-route="messages">E-mail</button></div><div class="fixture-list">${events}</div></article><article class="panel"><div class="row space"><div><span class="tag">Decisões do técnico</span><h2>Impacto imediato</h2></div><button class="secondary-btn mini" data-route="pressConference">Coletiva</button></div><div class="squad-ai-decision-grid-">${decisions}</div></article></section>
  <section class="panel"><div class="row space"><div><span class="tag">Celular</span><h2>Regras segurança</h2></div><button class="secondary-btn mini" data-route="mobileAudit">Verificação</button></div><ul class="small-list squad-ai-rules-">${rules}</ul></section>
