...
import { validateAssetIntegritySystem } from '../core/safety/asset-integrity-validator.js';
...
import { buildSaveSlotsV2Snapshot } from './systems/saveSlotsEngine.js';
import { buildLiveCalendarSnapshot } from './systems/liveCalendarEngine.js';
...
import { buildBetaProfessionalSnapshot } from './systems/betaProfessionalEngine.js';
import { buildAssetIntegritySnapshot } from './systems/assetIntegrityEngine.js';
...
async function boot(){
  ...
  const modules = {
    ...
    calendar:['Calendário Vivo','Sequência de jogos, viagens, fadiga, recuperação e risco físico'],
    scoutingCenter:['Scout Profissional','Observadores por região, relatórios, potencial, risco, custo e lista de desejos'],
    formation:['Formação','Escalação e desenho tático'],
    instructions:['Instruções','Comportamento coletivo e individual'],
    training:['Treino Semanal','Microciclo realista, recuperação, tático, físico, bola parada, finalização, coletivo e descanso'],
