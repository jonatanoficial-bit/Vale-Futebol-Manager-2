...
function saveCenterScreen(state={}){
  ...
  const careerSlotList = careerSlots.length ? careerSlots.slice(0,5).map(item=>`<article class="candidate-card save-career-slot-v731"><div><span class="tag">${item.slot === (save.activeSlot || 'principal') ? 'Ativo' : 'Slot'}</span><h3>${String(item.slot || 'principal').replace('principal','Carreira principal').replace('career-2','Carreira 2').replace('career-3','Carreira 3').replace('career-4','Carreira 4').replace('career-5','Carreira 5')}</h3><p>${item.manager || 'Manager'} · ${item.clubId || 'clube'} · ${item.updatedAt ? String(item.updatedAt).slice(0,10) : 'sem data'}</p></div><div class="candidate-side"><button class="main-btn mini" data-action="load-save-slot" data-slot="${item.slot}">Entrar</button><button class="secondary-btn mini" data-action="save-current-slot" data-slot="${item.slot}">Salvar aqui</button></div></article>`).join('') : '<p class="muted">Nenhum slot listado ainda. Crie uma nova carreira na central inicial.</p>';
