...
import { saveIntegritySnapshot, SAVE_MANAGER_VERSION, listSlots } from '../systems/saveManager.js';
...
import { buildGuidedTutorialSnapshot } from '../systems/guidedTutorialEngine.js';
import { buildManagerProgressionSnapshot } from '../systems/managerProgressionEngine.js';
import { buildManagerJobMarketSnapshot } from '../systems/managerJobMarketEngine.js';
...
function content(route,state={}){
  ...
  if(route==='managerProgression') return managerProgressionScreenV595(state);
  ...
  if(route==='standings') {
    ...
    return `<section class="standings-v080">
      ...
      <section class="panel table-panel"><div class="row space"><div><span class="tag">Tabela</span><h2>Classificação completa</h2></div><span class="status-pill">Atualizado</span></div><div class="table-scroll"><table class="table standings-table"><thead><tr><th>Pos</th><th>Time</th><th>J</th><th>V</th><th>E</th><th>D</th><th>GP</th><th>GC</th><th>SG</th><th>PTS</th><th>Forma</th></tr></thead><tbody>${table}</tbody></table></div></section>
  ...
  if(route==='championship') {
    ...
    return `<section class="championship-v070">
      <div class="panel championship-hero"><div><span class="tag">Agenda anual esportiva</span><h1>Competições do clube</h1><p class="small">Visão executiva das competições que o time disputa, agora com Copa do Brasil, Libertadores, Sul-Americana, Mundial/Intercontinental, impacto financeiro e reputação global.</p></div><div class="hero-actions"><button class="main-btn" data-route="copaDoBrasil">Copa do Brasil</button><button class="secondary-btn" data-route="worldCompetitions">Libertadores/Sul-Americana</button><button class="secondary-btn" data-route="calendar">Calendário</button></div></div>
...
function assetChecklistScreen(state={}){
  ...
  const priority = [
    ['Fundos principais', 'imagens/backgrounds/bg-cover.jpg', 'imagens/backgrounds/bg-lobby.jpg', 'imagens/backgrounds/bg-match.jpg'],
    ['Clube escolhido', `imagens/clubs/brazil/${state.clubId || 'santos'}/logo.png`, `imagens/stadiums/brazil/${state.clubId || 'santos'}.jpg`, `imagens/clubs/brazil/${state.clubId || 'santos'}/home-kit.png`],
    ['Jogadores', 'imagens/players/brazil/santos/joao-paulo.png', 'imagens/players/brazil/santos/giuliano.png', 'imagens/players/brazil/santos/guilherme.png'],
    ['Bandeiras', 'imagens/countries/br.png', 'imagens/countries/ar.png', 'imagens/countries/gb.png'],
    ['Patrocinadores', 'imagens/sponsors/umbro.png', 'imagens/sponsors/binance.png', 'imagens/sponsors/pixbet.png']
  ];
  ...
  return `<section class="asset-v230">
    <div class="panel asset-hero"><div><span class="tag">Imagens do jogo</span><h1>Checklist visual e caminhos oficiais</h1><p class="small">O jogo agora carrega imagens por caminhos padronizados, usa carregamento seguro, protege imagens quebradas e mantém proteção automática para clubes, jogadores, bandeiras, ligas, estádios, patrocinadores, staff e fundos.</p></div><div class="asset-hero-kpis"><strong>${summary.total}</strong><span>caminhos mapeados</span><small>${summary.proteções} proteções</small></div></div>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Mapa de imagens</h2></div><span class="status-pill">Carregadas: ${summary.cached}</span></div>${folders}<p class="alert">Você pode adicionar imagens no GitHub usando exatamente estes caminhos. Se a imagem não existir, o jogo usa imagem reserva automaticamente sem quebrar a tela.</p></article><article class="panel"><div class="row space"><div><span class="tag">Proteções</span><h2>Proteção de imagem</h2></div><span class="status-pill">Ativo</span></div><div class="asset-fallback-grid">${['club','player','country','stadium','background','sponsor','staff','competition'].map(t=>`<div>${safeImg(fallback(t), t, t, 'asset-fallback-icon')}<span>${t}</span></div>`).join('')}</div></article></section>
...
function saveCenterScreen(state={}){
  ...
  let careerSlots = [];
  try { careerSlots = listSlots(); } catch(err){ careerSlots = []; }
  ...
  const careerSlotList = careerSlots.length ? careerSlots.slice(0,3).map(item=>`<article class="candidate-card save-career-slot-v731"><div><span class="tag">${item.slot === (save.activeSlot || 'principal') ? 'Ativo' : 'Slot'}</span><h3>${String(item.slot || 'principal').replace('principal','Carreira principal').replace('career-2','Carreira 2').replace('career-3','Carreira 3')}</h3><p>${item.manager || 'Manager'} · ${item.clubId || 'clube'} · ${item.updatedAt ? String(item.updatedAt).slice(0,10) : 'sem data'}</p></div><div class="candidate-side"><button class="main-btn mini" data-action="load-save-slot" data-slot="${item.slot}">Entrar</button>${item.slot === (save.activeSlot || 'principal') ? `<button class="secondary-btn mini" data-action="save-current-slot" data-slot="${item.slot}">Atualizar</button>` : ''}</div></article>`).join('') : '<p class="muted">Nenhum slot listado ainda. Crie uma nova carreira na central inicial.</p>';
  const slots = [1,2,3,4,5].map(slot=>`<article class="candidate-card"><div><span class="tag">Backup ${slot}</span><h3>Backup seguro ${slot}</h3><p>Cópia rápida do slot ativo antes de testar builds, importar elencos ou avançar temporada.</p></div><div class="candidate-side"><button class="main-btn mini" data-action="save-backup" data-slot="${slot}">Criar</button><button class="secondary-btn mini" data-action="save-restore" data-slot="${slot}">Restaurar</button></div></article>`).join('');
  ...
  return `<section class="save-center-v510">
    <div class="panel championship-hero"><div><span class="tag">Save profissional ${SAVE_MANAGER_VERSION}</span><h1>Central Técnica de Save</h1><p class="small">Backups, exportação/importação e recuperação técnica. Para trocar de carreira, use a Central de Slots 2.0.</p></div><strong class="grade">${stability.health || 'Excelente'}</strong></div>
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Carreiras</span><h2>Slots jogáveis</h2></div><button class="main-btn mini" data-route="saveSlotsV2">Abrir Slots 2.0</button></div><div class="candidate-list">${careerSlotList}</div><div class="slot-create-row-v731"><button class="secondary-btn mini" data-action="new-career-slot" data-slot="principal">Nova no principal</button><button class="secondary-btn mini" data-action="new-career-slot" data-slot="career-2">Nova no slot 2</button><button class="secondary-btn mini" data-action="new-career-slot" data-slot="career-3">Nova no slot 3</button></div></article><article class="panel"><div class="row space"><div><span class="tag">Verificação</span><h2>Anti-quebra do save</h2></div><strong class="grade">6/6</strong></div>${checklist}</article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Backups manuais</span><h2>Cópias rápidas do slot ativo</h2></div><button class="secondary-btn mini" data-route="lobby">Lobby</button></div><div class="candidate-list">${slots}</div></article><article class="panel"><div class="row space"><div><span class="tag">Sair com segurança</span><h2>Trocar de carreira sem apagar</h2></div><button class="main-btn mini" data-action="exit-career">Salvar e sair para slots</button></div><p class="alert">Use “Salvar e sair” para voltar à central inicial e carregar outro slot. Resetar dados locais continua disponível só em Configurações.</p></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Exportar</span><h2>Guardar save fora do navegador</h2></div><button class="main-btn mini" data-action="save-export">Gerar JSON</button></div><textarea id="saveExportBox" class="save-textarea" imagem reserva="Clique em Gerar JSON e copie o texto. O arquivo contém envelope, versão, schema e estado normalizado."></textarea></article><article class="panel"><div class="row space"><div><span class="tag">Importar</span><h2>Restaurar por JSON</h2></div><button class="secondary-btn mini" data-action="save-import">Importar seguro</button></div><textarea id="saveImportBox" class="save-textarea" imagem reserva="Cole aqui o JSON exportado. JSON inválido será bloqueado sem derrubar o jogo."></textarea></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Autosave</span><h2>Últimos checkpoints</h2></div><span class="status-pill">${snapshot.hasAutoBackup?'Backup auto OK':'Aguardando'}</span></div><div class="news-list">${autoList}</div></article><article class="panel"><div class="row space"><div><span class="tag">Política de estabilidade</span><h2>Regras da build</h2></div><strong class="grade">AAA SAFE</strong></div><ul class="policy-list"><li>Nenhum save antigo deve quebrar a carreira.</li><li>Importação inválida é bloqueada antes de sobrescrever o progresso.</li><li>Backup automático tenta recuperar corrupção de JSON.</li><li>Slots manuais protegem testes de temporada e elencos.</li><li>A interface do jogador permanece limpa, sem logs técnicos.</li></ul><p class="alert">Sistema de saves pronto para carreiras longas.</p></article></section>
  </section>`;
}
...
function aiBalanceScreen(state={}){
  ...
  return `<section class="ai-balance-v190">
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Coerência esportiva</h2></div><span class="status-pill">Anti-quebra ativo</span></div><div class="needs-list">${diagnostics}</div></article><article class="panel"><div class="row space"><div><span class="tag">Últimas partidas</span><h2>Log do balanceamento</h2></div><button class="secondary-btn mini" data-route="standings">Classificação</button></div><div class="news-list">${logs}</div><ul class="small-list">${notes}</ul></article></section>
  ...
  const current = snap.roles.map(r=>`<article class="staff-role-card-v780">
    ...
    <div>
      ...
      <div class="staff-member-mini-v780">
        ${safeImg(r.member?.photo || 'imagens/imagem reservas/staff-generic.png','staff',r.member?.name || r.role,'staff-photo-v780')}
...
function messagesScreen(state={}){
  ...
  return `<section class="career-v150">
    ...
    <section class="panel"><div class="row space"><div><span class="tag">Radar profissional</span><h2>Propostas, sondagens e contratos</h2></div><button class="secondary-btn mini" data-route="club">Ver diretoria</button></div><div class="offer-list">${offers}</div></section>
  </section>`;
}
...
function trainingScreen(state={}){
  ...
  const activePreset = snapshot.preset?.id || state.training?.weeklyPreset || 'balanced';
  ...
  const presetCards = weeklyTrainingPresets.map(p=>`<button class="training-preset-card-v770 ${p.id===activePreset?'active':''}" data-action="training-set-preset" data-preset="${p.id}"><strong>${p.name}</strong><small>${p.description}</small></button>`).join('');
  const sessions = snapshot.sessions.map(s=>`<button class="microcycle-session-v770" data-action="training-apply-session" data-session="${s.id}">
    <div class="session-top"><span class="session-icon">${s.icon}</span><strong>${s.day}</strong><em>${s.type}</em></div>
    <h3>${s.title}</h3><small>${s.subtitle}</small><p>${s.goal}</p>
    <div class="meter"><span style="width:${s.load}%"></span></div><small>Carga ${s.load}% · clique para aplicar esta sessão</small>
  </button>`).join('');
  ...
  const history = (snapshot.training.microcycleHistory||[]).slice(-7).reverse().map(h=>`<div class="log-row"><strong>${h}</strong><small>registro interno salvo no slot atual</small></div>`).join('') || '<p class="small">Nenhuma semana aplicada ainda. Use o botão para avançar o microciclo completo.</p>';
  const sessionHistory = (snapshot.training.sessionHistory||[]).slice(-6).reverse().map(h=>`<div class="log-row"><strong>${h}</strong><small>sessão individual aplicada</small></div>`).join('') || '<p class="small">Clique em uma sessão do microciclo para testar impacto diário.</p>';
  const medical = (snapshot.training.medicalLog||[]).slice(-5).reverse().map(h=>`<div class="training-alert"><span>Médico</span><strong>${h}</strong><p>Controle preventivo ligado ao calendário vivo e ao motor de partida.</p></div>`).join('') || alerts;
  const impactRows = [
    ['Ataque', snapshot.matchImpact.attack, 'Finalização e criação'],
    ['Defesa', snapshot.matchImpact.defense, 'Organização sem bola'],
    ['Bola parada', snapshot.matchImpact.setPieces, 'Escanteios/faltas'],
    ['Controle tático', snapshot.matchImpact.tacticalControl, 'Posse, pressão e transição'],
    ['Condição', snapshot.matchImpact.fitness, 'Fôlego no jogo'],
    ['Risco físico', snapshot.matchImpact.injury, 'Lesão durante a partida']
  ].map(([label,value,small])=>`<div class="match-impact-card-v770"><span>${label}</span><strong>${Number(value)>=0?'+':''}${value}</strong><small>${small}</small></div>`).join('');
  return `<section class="training-v110 weekly-training-v770">
    <div class="panel weekly-training-hero-v770"><div><span class="tag">Treino semanal</span><h1>Microciclo profissional de 7 dias</h1><p class="small">Recuperação, tático, físico, bola parada, finalização, coletivo e descanso agora formam uma semana real. O treino altera fadiga, risco de lesão, prontidão e impacto direto no motor da partida.</p></div><div class="row gap"><button class="main-btn" data-action="training-apply-week">Aplicar microciclo completo</button><button class="secondary-btn" data-route="calendar">Calendário vivo</button><button class="secondary-btn" data-route="match">Testar no jogo</button></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Prontidão de jogo</span><strong>${snapshot.readiness}%</strong><small>${snapshot.recommendation}</small></div><div class="card kpi-card"><span>Carga semanal</span><strong>${snapshot.weeklyLoad}%</strong><small>${snapshot.preset?.name || 'Plano customizado'}</small></div><div class="card kpi-card"><span>Risco de lesão</span><strong>${snapshot.injuryRisk}%</strong><small>${snapshot.medicalStatus}</small></div><div class="card kpi-card"><span>Química coletiva</span><strong>${snapshot.collectiveChemistry}%</strong><small>entrosamento e automações</small></div></section>
    <section class="panel"><div class="row space"><div><span class="tag">Presets de semana</span><h2>Escolha o tipo de microciclo</h2></div><span class="status-pill">Semana ${snapshot.training.week}</span></div><div class="training-preset-grid-v770">${presetCards}</div></section>
    <section class="panel"><div class="row space"><div><span class="tag">Microciclo real</span><h2>7 sessões com impacto próprio</h2></div><strong class="grade">${snapshot.version}</strong></div><div class="microcycle-real-grid-v770">${sessions}</div></section>
    <section class="panel"><div class="row space"><div><span class="tag">Impacto direto no jogo</span><h2>O que muda na partida</h2></div><button class="secondary-btn mini" data-route="match">Ir para jogo</button></div><div class="match-impact-grid-v770">${impactRows}</div><div class="training-note"><strong>Leitura do auxiliar:</strong> ${snapshot.recommendation}</div></section>
    <section class="grid grid-2 training-main-grid"><article class="panel"><div class="row space"><div><span class="tag">Tema técnico</span><h2>Foco da semana</h2></div><span class="status-pill">${active.name}</span></div><div class="training-theme-list">${themeButtons}</div><div class="training-note"><strong>Impacto atual:</strong> ${active.focus}. Ganho principal: ${active.gain}. Carga estimada: ${active.fatigue}%.</div></article><article class="panel"><div class="row space"><div><span class="tag">Plano anterior preservado</span><h2>Resumo legado compatível</h2></div><strong class="grade">7 dias</strong></div><div class="week-plan">${legacyWeek}</div></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Desenvolvimento</span><h2>Jogadores em foco</h2></div><strong class="grade">Progressão real</strong></div><div class="development-list">${focus}</div></article><article class="panel"><div class="row space"><div><span class="tag">Staff</span><h2>Influência técnica</h2></div><button class="secondary-btn mini" data-route="staff">Gerir staff</button></div><div class="staff-impact-list">${staff}</div></article></section>
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Histórico semanal</span><h2>Microciclos aplicados</h2></div><span class="status-pill">Save local</span></div><div class="training-log-v770">${history}</div></article><article class="panel"><div class="row space"><div><span class="tag">Sessões individuais</span><h2>Diário de treino</h2></div><span class="status-pill">Teste fino</span></div><div class="training-log-v770">${sessionHistory}</div></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Regras realistas</span><h2>Controle de carga</h2></div><button class="secondary-btn mini" data-route="calendar">Ver agenda</button></div><div class="training-alerts">${rules}</div></article><article class="panel"><div class="row space"><div><span class="tag">Alertas</span><h2>Recomendações do centro</h2></div><strong class="grade">OK</strong></div><div class="training-alerts">${alerts}</div></article></section>
  </section>`;
}
...
function visualLibraryScreen(state={}){
  ...
  return `<section class="visual-library-v260 stack">
    <div class="panel"><div class="row space"><div><span class="tag">v2.6.1</span><h1>Biblioteca Visual Dinâmica</h1><p class="small">O jogo agora entende os fundos extras, logos organizados, países e ligas pelo mapa <code>data/asset-library.json</code>. Se uma imagem estiver ausente, quebrada ou com nome errado, o fallback preserva a tela.</p></div><strong class="grade">${summary.version}</strong></div></div>
    ...
    <section class="panel"><h2>Como adicionar novos visuais sem mexer no código</h2><p class="small">Suba a imagem no caminho indicado pelo guia e cadastre no <code>data/asset-library.json</code>. Para logos, use <code>imagens/clubs/pais/clube/logo.png</code> e <code>badge.png</code>. O jogo tentará a imagem, depois o fundo principal da tela, depois o fallback global.</p></section>
  </section>`;
}
...
function offerCardV310(o){
  ...
  const canAccept = o.canAccept !== false && (Number(o.fit||0) >= 45 || String(o.status||'').toLowerCase().includes('proposta'));
  const wage = Number(o.wage||0);
  const risk = o.negotiationRisk || 'médio';
  return `<article class="offer-row career-offer-card job-offer-card ${isNational?'national-offer':'club-offer'}">
    ${safeImg(country(o.country||'br'),'flag',o.country||'br','mini-flag')}
    <div><span class="tag">${isNational?'Seleção nacional':'Clube'} · ${o.status||'Sondagem'}</span><h3>${o.name}</h3><p>${o.role||'Treinador'} · ${o.objective||'Projeto esportivo'}</p><small>${o.calendar || o.league || 'Calendário completo'} · Pressão ${o.pressure||'Média'} · Reputação exigida ${o.requiredRep||40}</small>
      <div class="offer-money"><span><small>Salário</small><strong>€ ${wage.toFixed(2)}M/mês</strong></span><span><small>Contrato</small><strong>${o.months||24} meses</strong></span><span><small>Risco</small><strong>${risk}</strong></span></div>
      <p class="small">${o.project || o.rumor || 'Projeto esportivo monitorado pelo mercado.'}</p>
    </div>
...
function messagesScreenV310(state={}){
  ...
  <section class="panel"><div class="row space"><div><span class="tag">Radar profissional</span><h2>Propostas, sondagens e contratos</h2></div><button class="secondary-btn mini" data-action="career-offers-generate">Atualizar radar</button></div><div class="offer-list">${offers}</div></section>
...
function careerOffersScreenV310(state={}){
  ...
  const market = buildManagerJobMarketSnapshot(state);
  const unread = inboxMessages.filter(m=>m.status==='Novo').length + (state.career?.jobOffers?.length || 0);
  const important = inboxMessages.filter(m=>m.priority==='Alta').length;
  const activeJob = state.career?.nationalTeamJob;
  const offers = careerOffersFromState(state).map(offerCardV310).join('');
  ...
  return `<section class="career-v440 job-market-v596 stack"><div class="panel career-hero"><div><span class="tag">Mercado de treinadores v5.9.6</span><h1>Propostas reais e carreira profissional</h1><p class="small">Clubes e seleções agora avaliam reputação, nível, pressão, encaixe, salário, projeto, contrato e momento da carreira antes de formalizar convite.</p></div><div class="hero-actions"><button class="main-btn" data-action="career-offers-generate">Atualizar radar</button><button class="secondary-btn" data-action="manager-board-review">Revisão da diretoria</button><button class="secondary-btn" data-route="nationalTeam">Seleções</button></div></div>
  <section class="grid desktop-4"><div class="card kpi-card"><span>Poder de mercado</span><strong>${market.power}</strong><small>${market.negotiationMood}</small></div><div class="card kpi-card"><span>Ofertas ativas</span><strong>${market.activeOffers}</strong><small>${market.formalOffers} formais</small></div><div class="card kpi-card"><span>Clube atual</span><strong>${market.currentClub}</strong><small>salário € ${Number(market.currentWage||0).toFixed(2)}M/mês</small></div><div class="card kpi-card"><span>Melhor encaixe</span><strong>${market.bestOffer?.fit||0}%</strong><small>${market.bestOffer?.name||'Aguardando radar'}</small></div></section>
  <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Rumores</span><h2>Imprensa e bastidores</h2></div><span class="status-pill">vivo</span></div><div class="rumor-tape">${market.rumors.map(r=>`<div class="news-item"><strong>Mercado</strong><span>${r}</span></div>`).join('')}</div></article><article class="panel"><div class="row space"><div><span class="tag">Decisões</span><h2>Histórico de negociações</h2></div><span class="status-pill">${market.decisionLog.length}</span></div><div class="decision-log">${market.decisionLog.slice(0,7).map(d=>`<div class="timeline-row"><strong>•</strong><span>${d}</span><small>Mercado</small></div>`).join('') || '<p class="small">Nenhuma decisão registrada ainda.</p>'}</div></article></section>
...
function polishCenterScreenV340(state={}){
  ...
  return `<section class="polish-v340">
    <div class="panel polish-hero"><div><span class="tag">Visual do jogo</span><h1>Visual e leitura mobile</h1><p class="small">Esta central resume visual, desempenho, imagens, saves, navegação e compatibilidade em celular e PC.</p></div><div class="release-score"><strong>${audit.score}%</strong><small>${audit.status}</small></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Bloqueios críticos</span><strong>${audit.blockers}</strong><small>meta: zero</small></div><div class="card kpi-card"><span>Touch mobile</span><strong>44px+</strong><small>botões seguros</small></div><div class="card kpi-card"><span>Fullscreen</span><strong>100svh</strong><small>safe-area ativo</small></div><div class="card kpi-card"><span>Publicação</span><strong>OK</strong><small>Online</small></div></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Checklist</span><h2>Verificação visual e técnica</h2></div><span class="status-pill">${releaseReadiness.label}</span></div><div class="ux-audit-list">${rows}</div></article><article class="panel"><div class="row space"><div><span class="tag">Refinamentos</span><h2>O que foi melhorado</h2></div><strong class="grade">AAA</strong></div><ul class="premium-list bullets">${notes}</ul><div class="training-note"><strong>Recomendação:</strong> ${audit.recommendation}</div></article></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Telas críticas</span><h2>Teste rápido</h2></div><span class="status-pill">Modo seguro</span></div><div class="quick-route-grid"><button class="secondary-btn" data-route="lobby">Lobby</button><button class="secondary-btn" data-route="seasonCenter">Temporada</button><button class="secondary-btn" data-route="match">Partida</button><button class="secondary-btn" data-route="transfers">Transferências</button><button class="secondary-btn" data-route="careerOffers">Propostas</button><button class="secondary-btn" data-route="financeCenter">Economia</button></div></article><article class="panel"><div class="row space"><div><span class="tag">Próximos upgrades</span><h2>Expansão após v3.4</h2></div><button class="secondary-btn mini" data-route="visualLibrary">Imagens</button></div><div class="news-list compact"><div class="news-item"><strong>Base e categorias</strong><span>Sub-20, joias, promoção ao profissional e venda futura.</span></div><div class="news-item"><strong>Banco global</strong><span>Mais ligas, jogadores, staff e rankings internacionais.</span></div><div class="news-item"><strong>Experiência premium</strong><span>Sons, vibração opcional, animações e APK futuro.</span></div></div></article></section>
  </section>`;
}
...
function mobileAuditScreenV350(state={}){
  ...
  return `<section class="mobile-audit-v350 stack">
    <div class="panel polish-hero"><div><span class="tag">Teste de jogabilidade no celular</span><h1>Teste do jogo</h1><p class="small">Teste o fluxo completo: novo jogo, escolha de clube, partida, pós-jogo, tabela, transferências, save e navegação mobile.</p></div><div class="release-score"><strong>${report.score}%</strong><small>${report.status}</small></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Bloqueios</span><strong>${report.blockers}</strong><small>meta: zero</small></div><div class="card kpi-card"><span>Avisos</span><strong>${report.warnings}</strong><small>pontos de observação</small></div><div class="card kpi-card"><span>Telas críticas</span><strong>${report.routes.length}</strong><small>smoke test manual</small></div><div class="card kpi-card"><span>Dispositivos</span><strong>${report.devices.length}</strong><small>perfis mobile/desktop</small></div></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Resumo</span><h2>Estado atual da carreira</h2></div><span class="status-pill">${report.recommendation}</span></div><div class="ux-audit-list">${auditRows}</div></article><article class="panel"><div class="row space"><div><span class="tag">Correções</span><h2>O que esta build protege</h2></div><strong class="grade">OK</strong></div><ul class="premium-list bullets">${fixes}</ul><div class="training-note"><strong>Orientação:</strong> teste primeiro em celular real, depois publique online.</div></article></section>
    ...
    <section class="panel"><div class="row space"><div><span class="tag">Smoke test</span><h2>Telas principais</h2></div><span class="small">Use estes botões para abrir as telas de maior risco sem voltar ao menu.</span></div><div class="route-smoke-grid">${routeRows}</div></section>
  </section>`;
}
...
function data2026ScreenV360(state={}){
  ...
  return `<section class="data2026-v360 stack">
    ...
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Gerador de template</span><h2>Modelo de elenco por clube</h2></div><select data-action="set-ui-select" data-ui-key="dataClub">${options}</select></div><p class="small">Copie este modelo para <code>data/rosters/2026/${selectedClub}.json</code> e troque nomes/atributos. A estrutura é validada pela tela Atualizar elenco.</p><textarea class="code-box" readonly>${sample}</textarea></article><article class="panel"><div class="row space"><div><span class="tag">Avatares dos jogadores</span><h2>Caminho oficial</h2></div><button class="secondary-btn mini" data-route="rosterUpdate">Atualizar elenco</button></div><p class="small">Para cada jogador, use o mesmo <code>id</code> do JSON:</p><pre class="code-box small-code">imagens/players/brazil/${selectedClub}/id-do-jogador.png</pre><p class="alert">Se a imagem não existir, o jogo usa <code>imagens/imagem reservas/player-generic.png</code>. Portanto o elenco não quebra por falta de foto.</p></article></section>
  </section>`;
}



function managerProgressionScreenV595(state={}){
  const snap = buildManagerProgressionSnapshot(state);
  const p = snap.progression;
  const nextNeed = Math.max(0, Number(p.nextLevelXp||p.xp)-Number(p.xp||0));
  const specialties = snap.availableSpecialties.map(sp=>`<article class="card specialty-card ${sp.selected?'selected':''} ${sp.unlocked?'':'locked'}"><div class="row space"><div><span class="tag">${sp.icon} ${sp.unlocked?'Disponível':'Nível '+sp.unlockLevel}</span><h3>${sp.name}</h3></div><span class="status-pill">${sp.selected?'Foco atual':sp.unlocked?'Liberada':'Bloqueada'}</span></div><p class="small">${sp.desc}</p><button class="secondary-btn mini" data-action="manager-specialty" data-specialty="${sp.id}" ${sp.unlocked?'':'disabled'}>${sp.selected?'Selecionada':'Definir foco'}</button></article>`).join('');
  const achievements = snap.achievementCards.map(a=>`<article class="card achievement-card ${a.unlocked?'unlocked':''}"><div class="row space"><div><strong>${a.unlocked?'🏆':'🔒'} ${a.title}</strong><small>${a.desc}</small></div><span class="status-pill">${a.xp?`+${a.xp} XP`:'Marco'}</span></div></article>`).join('');
  const history = (p.history||[]).slice(-8).reverse().map(h=>`<div class="news-item"><strong>${h.date||'Carreira'}</strong><span>${h.label||h.reason} ${h.xp?`· +${h.xp} XP`:''}</span></div>`).join('') || '<div class="news-item"><strong>Carreira</strong><span>Ganhe XP jogando, treinando, respondendo coletivas e cumprindo missões.</span></div>';
  return `<section class="manager-progression-v595 stack">
    <div class="panel polish-hero xp-hero"><div><span class="tag">v5.9.5 · evolução do treinador</span><h1>XP, nível e licença do manager</h1><p class="small">O treinador agora evolui com ações reais da carreira. Partidas, coletivas, treino, tutorial e temporadas completas geram XP, liberam licenças e aumentam reputação.</p></div><div class="xp-ring" style="--xp-progress:${p.progress}"><div class="xp-ring-inner"><strong>${p.level}</strong><small>${p.progress}%</small></div></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Nível atual</span><strong>${p.level}</strong><small>${p.title}</small></div><div class="card kpi-card"><span>XP total</span><strong>${p.xp}</strong><small>${nextNeed} XP para o próximo nível</small></div><div class="card kpi-card"><span>Licença</span><strong>${p.license}</strong><small>${p.perk}</small></div><div class="card kpi-card"><span>Ranking</span><strong>${snap.ranking}</strong><small>${snap.rankingScore}/100</small></div></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Progresso</span><h2>Próxima licença</h2></div><span class="status-pill">${p.version}</span></div><div class="meter tall"><span style="width:${p.progress}%"></span></div><p class="small">XP do nível atual: ${p.xp - p.currentLevelXp}/${Math.max(1,p.nextLevelXp-p.currentLevelXp)}. A progressão melhora propostas, seleção nacional, confiança e reputação.</p><div class="row gap"><button class="main-btn mini" data-route="match">Ganhar XP jogando</button><button class="secondary-btn mini" data-action="manager-xp-simulate" data-xp="60" data-reason="teste interno de progressão">Teste +60 XP</button></div></article><article class="panel"><div class="row space"><div><span class="tag">Histórico</span><h2>Últimos ganhos</h2></div><span class="status-pill">${(p.history||[]).length}</span></div><div class="news-list compact xp-log">${history}</div></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Especialidades</span><h2>Escolha seu estilo de treinador</h2></div><span class="status-pill">${p.unlockedSpecialties.length} liberadas</span></div><div class="grid grid-2 specialty-grid">${specialties}</div></section>
    <section class="panel"><div class="row space"><div><span class="tag">Conquistas</span><h2>Marcos da carreira</h2></div><span class="status-pill">${snap.achievementCards.filter(a=>a.unlocked).length}/${snap.achievementCards.length}</span></div><div class="grid grid-2">${achievements}</div></section>
  </section>`;
}
...
function careerTutorialScreenV592(state={}){
  ...
  const guided = buildGuidedTutorialSnapshot(state);
  const current = guided.currentStep || guided.steps[0];
  const story = (state.career?.activeStory || []).slice(0,5).map(x=>`<div class="news-item"><strong>Carreira</strong><span>${x}</span></div>`).join('') || '<div class="news-item"><strong>Carreira</strong><span>Comece pelo tutorial guiado e construa sua reputação.</span></div>';
  const stepCards = guided.steps.map(step=>`<article class="card tutorial-step-card ${step.completed?'done':''} ${step.active?'active':''}">
    <div class="row space"><span class="tutorial-number">${step.completed?'✓':step.order}</span><span class="status-pill">${step.completed?'Concluída':step.active?'Agora':'Pendente'}</span></div>
    <h3>${step.title}</h3><p class="small">${step.short}</p>
    <div class="meter"><span style="width:${step.percent}%"></span></div>
    <p class="small"><strong>Recompensa:</strong> ${step.rewardText}</p>
    <div class="tutorial-card-actions"><button class="secondary-btn mini" data-route="${step.route}">${step.cta}</button><button class="main-btn mini" data-action="tutorial-complete" data-step="${step.id}" ${step.completed?'disabled':''}>${step.completed?'Recebido':'Concluir missão'}</button></div>
  </article>`).join('');
  const badges = guided.badges.map(b=>`<div class="badge-card ${b.unlocked?'unlocked':''}"><div><strong>${b.unlocked?'🏆':'🔒'} ${b.name}</strong><small>${b.text}</small></div><span class="status-pill">${b.unlocked?'Liberado':'Bloqueado'}</span></div>`).join('');
  const missions = buildCareerMissions(state);
  const missionCards = missions.slice(0,4).map(m=>`<article class="card mission-card ${m.done?'ok':'pending'}"><div class="row space"><div><strong>${m.done?'✅':'🎯'} ${m.title}</strong><small>${m.desc}</small></div><b>${m.progress}%</b></div><div class="meter"><span style="width:${m.progress}%"></span></div><p class="small">Recompensa: ${m.reward}</p></article>`).join('');
  const history = (state.career?.seasonHistory || []).slice(-4).reverse().map(h=>`<div class="news-item"><strong>${h.season} · ${h.clubName}</strong><span>${h.position}º lugar, ${h.points} pts · ${h.objective}</span></div>`).join('') || '<div class="news-item"><strong>Primeira temporada</strong><span>Ao terminar o ano, o jogo gera resumo e inicia a próxima temporada automaticamente.</span></div>';
  return `<section class="career-tutorial-v592 tutorial-v594 stack">
    <div class="panel polish-hero tutorial-hero"><div><span class="tag">v5.9.4 · Tutorial guiado premium</span><h1>Missões iniciais do treinador</h1><p class="small">Siga passos curtos para entender o modo carreira sem se perder: lobby, elenco, tática, coletiva, partida, relatório e save. Cada ação dá recompensas pequenas e fortalece a progressão.</p></div><div class="release-score"><strong>${guided.progress}%</strong><small>tutorial</small></div></div>
    <section class="grid desktop-4"><div class="card kpi-card"><span>Reputação</span><strong>${snap.reputation}</strong><small>técnico atual</small></div><div class="card kpi-card"><span>Etapas</span><strong>${guided.completedCount}/${guided.total}</strong><small>missões guiadas</small></div><div class="card kpi-card"><span>Renda acumulada</span><strong>€ ${snap.lifetimeEarnings}M</strong><small>modo infinito</small></div><div class="card kpi-card"><span>Temporada</span><strong>${snap.season}</strong><small>ano atual</small></div></section>
    <section class="grid grid-2"><article class="panel guide-panel"><div class="row space"><div><span class="tag">Próximo passo</span><h2>${current?.title || 'Tutorial concluído'}</h2></div><span class="status-pill">${guided.status==='completed'?'Completo':'Ativo'}</span></div><p>${current?.guide || 'Você concluiu o tutorial inicial. Agora avance na carreira, melhore sua reputação e busque propostas maiores.'}</p><div class="row gap"><button class="main-btn" data-route="${current?.route || 'lobby'}">${current?.cta || 'Voltar ao lobby'}</button><button class="secondary-btn" data-action="tutorial-complete" data-step="${current?.id || 'welcome-club'}">Marcar como concluído</button></div></article><article class="panel guide-panel"><div class="row space"><div><span class="tag">Recompensas</span><h2>Conquistas iniciais</h2></div><span class="status-pill">${guided.badges.filter(b=>b.unlocked).length}/${guided.badges.length}</span></div><div class="stack">${badges}</div></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Roteiro guiado</span><h2>Primeiras ações do manager</h2></div><span class="status-pill">${guided.version}</span></div><div class="grid grid-2">${stepCards}</div></section>
    <section class="grid grid-2"><article class="panel"><div class="row space"><div><span class="tag">Missões de carreira</span><h2>Metas permanentes</h2></div><span class="status-pill">${snap.completedMissions}/${snap.missionCount}</span></div><div class="grid grid-2">${missionCards}</div></article><article class="panel"><div class="row space"><div><span class="tag">Linha viva</span><h2>História do treinador</h2></div><button class="secondary-btn mini" data-route="careerOffers">Ver propostas</button></div><div class="news-list compact">${story}</div></article></section>
    <section class="panel"><div class="row space"><div><span class="tag">Modo infinito</span><h2>Temporadas encerradas</h2></div><span class="status-pill">sem limite de anos</span></div><div class="news-list compact">${history}</div><p class="alert success">Ao fechar os 38 jogos, o jogo calcula posição, prêmio, reputação, segurança no cargo e abre a temporada seguinte.</p></section>
  </section>`;
}
...
function databaseMay2026ScreenV460(state={}){
  ...
  return `<section class="database2026-v460 stack">
    ...
    <section class="panel"><div class="row space"><div><span class="tag">Elenco selecionado</span><h2>Prévia dos atletas</h2></div><span class="small">Fotos ausentes usam imagem reserva automaticamente.</span></div><div class="table-wrap"><table><thead><tr><th>Jogador</th><th>Pos</th><th>OVR</th><th>POT</th><th>Idade</th><th>Valor</th><th>Salário</th><th>Contrato</th></tr></thead><tbody>${rosterRows}</tbody></table></div></section>
...
function academyScoutingScreenV380(state={}){
  ...
  return `<section class="academy-v380 stack">
    ...
    <section class="panel"><div class="row space"><div><span class="tag">Manutenção segura</span><h2>Template editável de promessas</h2></div><span class="small">Fotos futuras em <code>imagens/players/youth/id-do-jogador.png</code></span></div><textarea class="code-box" readonly>${template}</textarea><p class="alert">Se uma foto de jovem não existir, o jogo usa o imagem reserva de jogador. Se um campo do JSON estiver incompleto, o motor normaliza dados e evita travamento.</p></section>
  </section>`;
}
