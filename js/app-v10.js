...
const SCHEMA = 1000;
const STORE_KEY = 'vale-futebol-manager-v10';
const LEGACY_KEY = 'vale-futebol-manager-v9';
const MAX_SLOTS = 3;
...
function worldDatabaseModal() {
  ...
  showModal('Base mundial 2026', '<div class="world-stats"><div><strong>' + stats.playableClubs + '</strong><span>clubes com elenco</span></div><div><strong>' + stats.simulationClubs + '</strong><span>clubes no mundo</span></div><div><strong>' + stats.clubPlayers.toLocaleString('pt-BR') + '</strong><span>jogadores de clubes</span></div><div><strong>' + stats.nationalTeams + '</strong><span>associações FIFA</span></div><div><strong>' + stats.officialNationalSquads + '</strong><span>seleções com elenco oficial</span></div><div><strong>' + stats.nationalPlayers.toLocaleString('pt-BR') + '</strong><span>jogadores de seleções</span></div><div><strong>' + countries + '</strong><span>países com liga</span></div><div><strong>' + stats.managerAvatars + '</strong><span>avatares</span></div></div><p class="muted">As 211 seleções participam das eliminatórias. As 48 convocatórias publicadas oficialmente para a Copa de 2026 podem ser comandadas; clubes sem elenco nominal verificado participam pela IA.</p>');
}
...
function renderManagerSetup() {
  const avatars = Array.from({length:36},(_,i)=>i+1).map(number => {
    const padded = String(number).padStart(2,'0');
    const primary = number <= 16 ? './assets/avatars/manager-' + padded + '.png' : './assets/avatars/manager-v801-' + String(number-16).padStart(2,'0') + '.png';
    return '<button class="avatar-btn ' + (session.selectedAvatar===number?'selected':'') + '" data-action="select-avatar" data-avatar="' + number + '" aria-label="Avatar ' + number + '"><img src="' + primary + '" alt="" onerror="__vfmFallback(event,\'avatar\')"></button>';
  }).join('');
...
function leagueSize(leagueId) {
  return ({'brasileirao-a':20,'brasileirao-b':20,'premier-league':20,'laliga':20,'bundesliga':18,'ligue-1':18,'liga-portugal':18,'serie-a-italia':20,'argentina-primera':28,'chile-primera':16,'colombia-primera-a':20,'ecuador-serie-a':16,'uruguay-primera':16})[leagueId] || 16;
}
...
function buildContinentalFixtures(club, startDate, participants) {
  ...
  const id=Object.entries(allocations).find(([,range])=>seed>=Number(range[0])&&seed<=Number(range[1]))?.[0];
  if(!id)return [];
...
function buildWorldFixtures(club, startDate) {
  ...
  if (club.rating < Math.max(80, (league?.baseRating || 75) + 2)) return [];
...
async function createCareer() {
  ...
  try {
    ...
    const fixtures = [...buildLeagueFixtures(session.selectedClub,participants,startDate),...buildCupFixtures(session.selectedClub,participants,startDate),...buildContinentalFixtures(session.selectedClub,startDate,participants),...buildWorldFixtures(session.selectedClub,startDate)].sort((a,b)=>new Date(a.date)-new Date(b.date));
    ...
    session.career = {
      ...
      manager:{name,avatar:session.selectedAvatar,difficulty:document.querySelector('#difficulty')?.value||'Equilibrado',reputation,xp:0,level:1},
      club:{...session.selectedClub},season:2026,date:startDate.toISOString(),week:1,budget,board:72,morale:74,fitness:88,
      roster,lineupIds:pickLineup(roster).map(p=>p.id),tactics:{formation:'4-3-3',mentality:'Equilibrada',pressure:58,tempo:55,width:55},
      ...
      staff:{assistant:68,fitnessCoach:66,scout:64,medical:65},lastTrainingWeek:0,national:null,seasonSummary:null
    };
...
function renderGame(content) {
  ...
  app.innerHTML = '<main class="screen game-screen world-game"><nav class="world-nav" aria-label="Menu principal"><div class="nav-brand">V</div>'+nav+'</nav><section class="game-stage"><header class="world-topbar"><div class="club-identity"><img src="./'+escapeHtml(c.club.badge)+'" alt="" onerror="__vfmFallback(event)"><span><strong>'+escapeHtml(c.club.name)+'</strong><small>'+escapeHtml(c.manager.name)+' · '+escapeHtml(c.club.leagueName)+'</small></span></div><div class="top-metrics"><span>Data <strong>'+formatDate(c.date)+'</strong></span><span>Reputação <strong>'+c.manager.reputation+'</strong></span><span>Saldo <strong>'+money(c.budget)+'</strong></span></div><button class="btn btn-small" data-action="save">Salvar</button></header><div class="game-content">'+content+'</div></section></main>';
}
...
function renderTactics() {
  ...
  return sectionHead('Tática e formação','Defina o desenho dos onze titulares e o comportamento coletivo.')+'<div class="tactics-layout"><article class="panel">'+formationPreview(c.roster,c.lineupIds,t.formation)+'</article><article class="panel tactics-controls"><div class="field"><label>Formação</label><select data-action="formation-select">'+Object.keys(FORMATIONS).map(f=>'<option '+(f===t.formation?'selected':'')+'>'+f+'</option>').join('')+'</select></div><div class="field"><label>Mentalidade</label><select data-action="mentality-select">'+['Defensiva','Equilibrada','Ofensiva'].map(m=>'<option '+(m===t.mentality?'selected':'')+'>'+m+'</option>').join('')+'</select></div><label>Pressão <output>'+t.pressure+'</output><input type="range" min="20" max="90" value="'+t.pressure+'" data-action="pressure-range"></label><label>Ritmo <output>'+t.tempo+'</output><input type="range" min="20" max="90" value="'+t.tempo+'" data-action="tempo-range"></label><p class="muted">A formação também organiza visualmente os 22 jogadores durante a partida.</p></article></div>';
}
...
function sortedTable() { return session.career.table.slice().sort((a,b)=>b.points-a.points||b.gd-a.gd||b.gf-a.gf||b.team.rating-a.team.rating); }
...
function rulesText(league) {
  ...
  if(rules.promotion)result.push('1º–'+rules.promotion+'º: acesso');
...
function renderTraining() {
  ...
  return sectionHead('Centro de treinamento','Defina um microciclo por semana e acompanhe o impacto no elenco.','<span class="tag">Físico '+c.fitness+'%</span>')+'<div class="training-grid">'+plans.map(([id,name,effect])=>'<button class="training-card" data-action="apply-training" data-plan="'+id+'" '+(trained?'disabled':'')+'><span>◎</span><strong>'+name+'</strong><small>'+effect+'</small></button>').join('')+'</div>'+(trained?'<div class="notice success">O treino desta semana já foi aplicado.</div>':'');
}
...
function renderNational() {
  ...
  if(!n){
    const filters=[['official','48 com elenco oficial'],['all','Todas as 211'],['CONMEBOL','CONMEBOL'],['UEFA','UEFA'],['CONCACAF','CONCACAF'],['AFC','AFC'],['CAF','CAF'],['OFC','OFC']];
    ...
    return sectionHead('Seleções nacionais','211 associações em seis confederações, com eliminatórias e carreira dupla.','<span class="tag">Sua reputação: '+c.manager.reputation+'</span>')+'<div class="notice">Todas participam da simulação. As 48 convocatórias oficiais da Copa de 2026 são comandáveis; o GER é um índice próprio do VFM.</div><div class="toolbar"><select data-action="national-filter" aria-label="Filtrar seleções">'+filters.map(([id,label])=>'<option value="'+id+'" '+(session.nationalFilter===id?'selected':'')+'>'+label+'</option>').join('')+'</select><input data-action="national-search" aria-label="Buscar seleção" placeholder="Buscar seleção" value="'+escapeHtml(session.nationalSearch)+'"><span class="tag">'+shown.length+' seleções</span></div><div class="national-grid">'+offers+'</div>';
  }
...
function tickMatch() {
  ...
  const bias=(session.career.tactics.mentality==='Ofensiva'?5:session.career.tactics.mentality==='Defensiva'?-3:0)+(teamOnHome()?3:-3);
...
function applyTraining(plan) {
  ...
  c.fitness=clamp(c.fitness+fitness,20,100);c.morale=clamp(c.morale+morale,20,100);c.roster.forEach(p=>{p.fitness=clamp(p.fitness+fitness,20,100);p.morale=clamp(p.morale+morale,20,100);});c.lastTrainingWeek=c.week;persist();renderGame(renderTraining());toast('Microciclo aplicado ao elenco.','success');
}

function buyPlayer(id) {
  const p=session.market.find(item=>item.id===id);if(!p)return;const cost=p.value*1000000;
  if(session.career.budget<cost){toast('Orçamento insuficiente.','error');return;}
  session.career.budget-=cost;session.career.roster.push({...p});session.career.ledger.push({date:new Date().toISOString(),label:'Contratação · '+p.name,amount:-cost,type:'expense'});session.market=session.market.filter(item=>item.id!==id);persist();renderGame(renderMarket());toast(p.name+' foi contratado.','success');
}
